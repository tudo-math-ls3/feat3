...
if ( (CMAKE_SYSTEM_NAME STREQUAL "Linux") OR
  ...
  set (GNU_VERSION_MAJOR 0)
  set (GNU_VERSION_MINOR 0)
  set (GNU_VERSION_MINORMINOR 0)
  ...
  string(REGEX REPLACE ".*([0-9]+)[.][0-9]+[.]+[0-9].*" "\\1" GNU_VERSION_MAJOR "${GNU_VERSION_STRING}")
  string(REGEX REPLACE ".*[0-9]+[.]([0-9]+)[.]+[0-9].*" "\\1" GNU_VERSION_MINOR "${GNU_VERSION_STRING}")
  string(REGEX REPLACE ".*[0-9]+[.][0-9]+[.]([0-9]+).*" "\\1" GNU_VERSION_MINORMINOR "${GNU_VERSION_STRING}")
  message (STATUS "Found GNU compiler suite v.${GNU_VERSION_MAJOR}.${GNU_VERSION_MINOR}.${GNU_VERSION_MINORMINOR}.")
  # and check for recent enough version
  if ("${GNU_VERSION_MAJOR}.${GNU_VERSION_MINOR}.${GNU_VERSION_MINORMINOR}" VERSION_LESS "4.4.0")
    message (STATUS "##############################################################")
    message (STATUS "GNU Compiler Version ${GNU_VERSION_MAJOR}.${GNU_VERSION_MINOR}.${GNU_VERSION_MINORMINOR} is not supported,")
...
if (FEAST_CXX_FLAGS_INTERNAL STREQUAL "")
  ...
  if (BUILD_ID MATCHES "^coverage-.+|.+-coverage-.+|.+-coverage$")
    set (FEAST_CXX_FLAGS_INTERNAL "${FEAST_CXX_FLAGS_INTERNAL} -fprofile-arcs -ftest-coverage")
  endif()

  if (FEAST_DEBUG_MODE)
    # unoptimised settings for all archs
    # the following flag might be useful: -m128bit-long-double
    # for older compilers enforce at minimum: -std=c++98
    set (FEAST_CXX_FLAGS_INTERNAL "${FEAST_CXX_FLAGS_INTERNAL} -O0 -Wall -Wextra -Wundef -g")
    ...
    # the following flag might be useful: -m128bit-long-double
    # for older compilers enforce at minimum: -std=c++98
    set (FEAST_CXX_FLAGS_INTERNAL "${FEAST_CXX_FLAGS_INTERNAL} -O3 -ffast-math -ggdb -foptimize-register-move -fprefetch-loop-arrays -funroll-loops -mfpmath=sse")
    ...
    elseif (FEAST_CPU_TYPE STREQUAL "nehalem")
      if ("${GNU_VERSION_MAJOR}.${GNU_VERSION_MINOR}.${GNU_VERSION_MINORMINOR}" VERSION_LESS "4.6.0")
    ...
    elseif (FEAST_CPU_TYPE STREQUAL "ivybridge")
      if ("${GNU_VERSION_MAJOR}.${GNU_VERSION_MINOR}.${GNU_VERSION_MINORMINOR}" VERSION_LESS "4.6.0")
        # note: older GCC versions do not support -match=corei7 , so emulate: core2+sse4.2=corei7
        set (FEAST_CXX_FLAGS_INTERNAL "${FEAST_CXX_FLAGS_INTERNAL} -march=core2 -msse4.2 -m64")
      else ()
        set (FEAST_CXX_FLAGS_INTERNAL "${FEAST_CXX_FLAGS_INTERNAL} -march=corei7 -m64 -msse4 -msse4.1 -msse4.2")
      endif ()
    elseif (FEAST_CPU_TYPE STREQUAL "sandybridge")
      if ("${GNU_VERSION_MAJOR}.${GNU_VERSION_MINOR}.${GNU_VERSION_MINORMINOR}" VERSION_LESS "4.6.0")
        # note: older GCC versions do not support -match=corei7 , so emulate: core2+sse4.2=corei7
        set (FEAST_CXX_FLAGS_INTERNAL "${FEAST_CXX_FLAGS_INTERNAL} -march=core2 -msse4.2 -m64")
      else ()
        set (FEAST_CXX_FLAGS_INTERNAL "${FEAST_CXX_FLAGS_INTERNAL} -march=corei7-avx -m64 -msse4 -msse4.1 -msse4.2")
      endif ()
    ...
    elseif (FEAST_CPU_TYPE STREQUAL "pentium4")
      set (FEAST_CXX_FLAGS_INTERNAL "${FEAST_CXX_FLAGS_INTERNAL} -march=pentium4m -m64")
...
if (NOT COMPILER_SUPPORTS_FLAG)
  ...
  message (STATUS "unsupported by the GNU Compiler v.${GNU_VERSION_MAJOR}.${GNU_VERSION_MINOR}.${GNU_VERSION_MINORMINOR}.")
  message (STATUS "##############################################################")
  message (FATAL_ERROR "")
endif ()
