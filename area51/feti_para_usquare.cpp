...
namespace FETI{
  ...
  public:
    ...
    std::shared_ptr<Umf> _local_solver;

    LocalSystem() = delete;

    template<typename Operator_ , typename Functional_>
    LocalSystem(SpaceType & space,
    ...
    : _floating{is_floating}
    {
      ...
      Assembly::SymbolicAssembler::assemble_matrix_std1(_matrix_local, space);
    ...
    Assembly::LinearFunctionalAssembler::assemble_vector(_vec_rhs, force_funk, space, cubature_factory);

    // Apply the filter onto the system matrix...
    filter.filter_mat(_matrix_local);
    ...
    filter.filter_rhs(_vec_rhs);
    filter.filter_sol(_vec_sol);

    if(_floating)
      _local_solver = std::make_shared<FloUmf>(_matrix_local, _R_vector);
    else
      _local_solver = std::make_shared<RegUmf>(_matrix_local);
    ...
    {
      _local_solver->apply(output, input);
    }
  ...
  public:
    ...
    */
    ...
    //whereby i denotes a possible edge between two domains and input_reduced is vector remaining
    ...
    void test_vector(const std::vector<LocalVectorType>& vec) const
    {
      ...
      for(std::size_t idx(0u); recv_reqs.wait_any(idx); )
      {
        ...
        {
          //XASSERTM(Math::abs(recv_bufs.at(idx)(i) - vec.at(idx)(i)) < tol_eps , "Values not the same!");
          XASSERTM(Math::abs(recv_views.at(idx)(i) - vi(i)) < tol_eps , "Values not the same!");
        }
      }
...
//       std::cout << "Finalizing Gate...\n";
  ...
  public:
    ...
    void gather_Q(const FETIGate& gate, const bool verbose = false)
    {
      ...
      auto rb = recv_buf.elements_view_w();
      //now post the igather request
      _comm->gather(send_buf.elements_view_r().get_r(), std::size_t(2*n), rb.get_w(), std::size_t(2*n), 0);
      if(verbose)
      ...
      if(_comm->rank() == 0)
      {
        ...
        for(IndexType i(0); i < dom_size ; ++i)
        {
          ...
          {
            factory.add(i, IndexType(rb(i*2*n + j)), rb(i*2*n + n + j));
            //if we read in the diagonal element, we stop, as rest of the data is junk
            if(IndexType(rb(i*2*n + j)) == i)
              break;
          }
        }
...
//       _comm->print("Residuum initialized!");
  ...
  void main(int argc, char* argv[])
  {
    ...
    watch.start();
    ts_1.stamp();
    LocalSystem local_sys(space, filter_local, cubature_factory, laplace_operator, force_functional, is_floating);
    ts_2.stamp();
    watch.stop();
    t = ts_2.elapsed(ts_1);
    comm.allreduce(&t, &t_max, std::size_t(1), Dist::op_max);
    ...
    watch.start();
    CG.solve(gate, local_sys, project, CG_plot);
    watch.stop();
  ...
  void error_analysis(std::vector<FETI::IndexType> lvl, std::vector<FETI::DataType> h0_error,
                     std::vector<FETI::DataType> h1_error, std::vector<FETI::DataType> rate_h0,
                     std::vector<FETI::DataType> rate_h1)
{
  Dist::Comm comm = Dist::Comm::world();
  comm.print("Convergence test for " + stringify(comm.size()) + " sub-domains:");
  comm.print("------------------------------------------");
  comm.print("Level     |      H0 Error      |    rate      |       H1 Error     |      rate");
  for(FETI::IndexType i(0); i < lvl.size(); ++i)
  {
    String long_string = stringify(lvl[i]) + "    |   " + stringify(h0_error[i]) + "        |    ";
    if(i==0)
      long_string += "---     |         ";
    else
      long_string += "  " + stringify(rate_h0[i]) + "   |    ";
    long_string += "  " + stringify(h1_error[i]) + "      |    ";
    if(i==0)
      long_string += "---             ";
    else
      long_string += "  " + stringify(rate_h1[i]) + "\n ";
    comm.print(std::move(long_string));
  }
}
