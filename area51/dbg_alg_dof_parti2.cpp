...
int main(int argc, char* argv[])
{
  ...
  typedef LAFEM::DenseVectorBlocked<DataType, IndexType, 2> LocalVectorType;
  ...
  typedef LAFEM::SparseMatrixBCSR<DataType, IndexType, 2, 2> LocalMatrixType;

  //typedef LAFEM::UnitFilterBlocked<DataType, IndexType, 2> LocalFilterType;
  typedef LAFEM::NoneFilterBlocked<DataType, IndexType, 2> LocalFilterType;
  ...
  for(auto it = neighbor_ranks.begin(); it != neighbor_ranks.end(); ++it)
  {
    ...
    Assembly::MirrorAssembler::assemble_mirror(neighbor_mirror, space_1, *neighbor_halo);
    gate.push(neighbor_rank, std::move(neighbor_mirror));
  }

  {
    gate.compile(LocalVectorType(space_1.get_num_dofs()));
  }
  ...
  /*{
    ...
    Index nn = matrix.local().used_elements();
    for(Index i = 0; i < nn; ++i)
    {
      val[i][0][0] = double(i) + 0.11;
      val[i][0][1] = double(i) + 0.12;
      val[i][1][0] = double(i) + 0.21;
      val[i][1][1] = double(i) + 0.22;
    }
  }*/
