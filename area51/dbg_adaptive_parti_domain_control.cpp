...
namespace DbgAdaptivePartiDomainControl
{
  ...
  void run(SimpleArgParser& args, Control::Domain::DomainControl<DomainLevel_>& domain)
  {
    ...
    //Analytic::Common::ConstantFunction<ShapeType::dimension> sol_func(5.0);
    ...
    const String cubature("auto-degree:3");

    for(Index i(0); i < num_levels; i++)
    {
      Geometry::ExportVTK<MeshType> exporter(domain.at(i).level().get_mesh());
      exporter.write("poisson_dirichlet_partial_lvl-" + stringify(i), domain.at(i).layer().comm());
    }
    ...
    for (Index i(0); (i < domain.size_physical()) && ((i+1) < domain.size_virtual()); ++i)
    {
      ...
      if((i+1) < domain.size_physical())
        system_levels.at(i)->assemble_transfer_adaptive(*system_levels.at(i+1), domain.at(i), domain.at(i+1), cubature);
      else
        system_levels.at(i)->assemble_transfer_adaptive(domain.at(i), domain.at(i+1), cubature);
    }
  ...
  void main(int argc, char* argv [])
  {
    ...
    const auto refinement_strategy = [](Geometry::SubdivisionLevels& sdls, const MeshType& mesh)
    {
      using VertexType = typename MeshType::VertexType;

      const auto& vertex_set = mesh.get_vertex_set();
      const VertexType center{0.5, 0.5};

      for(Index i(0); i < sdls.size(); i++)
      {
        if((vertex_set[i] - center).norm_euclid() < 0.25)
        {
          sdls[i] = 2;
        }
        else
    ...
    using SubDomainLevelType = Control::Domain::SimpleDomainLevel<MeshType, TrafoType, SpaceType>;
    using DomainLevelType = Control::Domain::AdaptiveLevelWrapper<SubDomainLevelType, Geometry::SunZhaoMaExpansionTemplates>;
    Control::Domain::AdaptivePartiDomainControl<DomainLevelType, Geometry::SunZhaoMaExpansionTemplates> domain(comm, true, refinement_strategy);
...
int main(int argc, char* argv [])
{
  ...
  try
  {
    DbgAdaptivePartiDomainControl::main(argc, argv);
  }
