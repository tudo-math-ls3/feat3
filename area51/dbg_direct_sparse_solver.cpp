...
namespace Tutorial06
{
  ...
  void main(Index level, String args)
  {
    ...
    // create a new direct sparse solver
    auto solver = Solver::new_direct_sparse_solver(matrix, filter);

    if(!args.empty())
    {
      comm.print("Setting allowed backends: '" + args + "'");
      solver->push_allowed_backend_list(args);
    }

    // Initialize the solver.
    solver->init();

    comm.print("Selected Solver: " + solver->name());
    ...
    if(false)
    {
      ...
      String vtk_name = String("./dbg-direct-sparse-solver.") + stringify(level) + "-n" + stringify(comm.size());
