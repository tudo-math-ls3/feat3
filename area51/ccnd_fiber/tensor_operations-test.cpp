...
public:
  // constructor
  explicit TensorOperatorTest(const std::string& test_name, PreferredBackend backend) :
  UnitTest(test_name, Type::Traits<DT_>::name(), Type::Traits<IT_>::name(), backend),
  _eps(Math::eps<DT_>())
  {
  }
  ...
  void test_symmetric_tensor_contraction_2D() const
  {
    ...
    //write values in
    t(0) = DT_(0.3); //A1111
    t(15) = DT_(-0.334); //A2222

    t(0*8 + 0*4 + 0*2 + 1) = t(0*8 + 0*4 + 1*2 + 0) = t(0*8 + 1*4 + 0*2 + 0) = t(1*8 + 0*4 + 0*2 + 0) = DT_(5.7); //A1112
    t(1*8 + 1*4 + 1*2 + 0) = t(1*8 + 1*4 + 0*2 + 1) = t(1*8 + 0*4 + 1*2 + 1) = t(0*8 + 1*4 + 1*2 + 1) = DT_(2.082); //A1222

    t(0*8 + 0*4 + 1*2 + 1) = t(0*8 + 1*4 + 0*2 + 1) = t(1*8 + 0*4 + 0*2 + 1) = t(1*8 + 1*4 + 0*2 + 0) = t(1*8 + 0*4 + 1*2 + 0) = t(0*8 + 1*4 + 1*2 + 0) = DT_(-1.77); //A1122, A1212 A2112 A2121 A2211 A1221
    ...
    //now test if the symmetric version is the same as our non symmetric version for a symmetric tensor
    {
      ...
      set_tensor4_outer_product_contraction_12(mat_A, a, b, t);
      mat_B = fourth_order_contraction(a, b, t_sym, DT_(1));
      for(int i(0); i < 2; ++i)
      {
        for(int j(0); j < 2; ++j)
        {
    ...
    //test whether order for symmteric variant is irrelevant
    {
      ...
      mat_A = fourth_order_contraction(b, a, t_sym, DT_(1.36));
      mat_B = fourth_order_contraction(a, b, t_sym, DT_(1.36));
      for(int i(0); i < 2; ++i)
      {
        for(int j(0); j < 2; ++j)
        {
  ...
  void test_symmetric_tensor_contraction_3D() const
  {
    ...
    //initialize the symmetric tensor as vector
    Vector<DT_, 81> t(DT_(0));
    //write values in
    t(0) = DT_(0.3); //A1111
    t(27 + 9 + 3 + 1) = DT_(-0.334); //A2222
    t(80) = DT_(0.243); //A3333

    t(0*27 + 0*9 + 0*3 + 1) = t(0*27 + 0*9 + 1*3 + 0) = t(0*27 + 1*9 + 0*3 + 0) = t(1*27 + 0*9 + 0*3 + 0) = DT_(5.7); //A1112
    t(0*27 + 0*9 + 0*3 + 2) = t(0*27 + 0*9 + 2*3 + 0) = t(0*27 + 2*9 + 0*3 + 0) = t(2*27 + 0*9 + 0*3 + 0) = DT_(1.3176); //A1113
    t(1*27 + 1*9 + 1*3 + 0) = t(1*27 + 1*9 + 0*3 + 1) = t(1*27 + 0*9 + 1*3 + 1) = t(0*27 + 1*9 + 1*3 + 1) = DT_(2.082); //A1222
    t(1*27 + 1*9 + 1*3 + 2) = t(1*27 + 1*9 + 2*3 + 1) = t(1*27 + 2*9 + 1*3 + 1) = t(2*27 + 1*9 + 1*3 + 1) = DT_(0.854); //A2223
    t(2*27 + 2*9 + 2*3 + 0) = t(2*27 + 2*9 + 0*3 + 2) = t(2*27 + 0*9 + 2*3 + 2) = t(0*27 + 2*9 + 2*3 + 2) = DT_(-1.302); //A1333
    t(2*27 + 2*9 + 2*3 + 1) = t(2*27 + 2*9 + 1*3 + 2) = t(2*27 + 1*9 + 2*3 + 2) = t(1*27 + 2*9 + 2*3 + 2) = DT_(-2.93); //A2333

    t(0*27 + 0*9 + 1*3 + 1) = t(0*27 + 1*9 + 0*3 + 1) = t(1*27 + 0*9 + 0*3 + 1) = t(1*27 + 1*9 + 0*3 + 0) = t(1*27 + 0*9 + 1*3 + 0) = t(0*27 + 1*9 + 1*3 + 0) = DT_(0.49965); //A1122, A1212 A2112 A2121 A2211 A1221
    t(0*27 + 0*9 + 2*3 + 2) = t(0*27 + 2*9 + 0*3 + 2) = t(2*27 + 0*9 + 0*3 + 2) = t(2*27 + 2*9 + 0*3 + 0) = t(2*27 + 0*9 + 2*3 + 0) = t(0*27 + 2*9 + 2*3 + 0) = DT_(-1.77); //A1133
    t(1*27 + 1*9 + 2*3 + 2) = t(1*27 + 2*9 + 1*3 + 2) = t(2*27 + 1*9 + 1*3 + 2) = t(2*27 + 2*9 + 1*3 + 1) = t(2*27 + 1*9 + 2*3 + 1) = t(1*27 + 2*9 + 2*3 + 1) = DT_(0.9534); //A2233
    ...
    {
      set_tensor4_outer_product_contraction_symmetric(mat_A, a, b, t_sym, DT_(1.34));
      mat_B = fourth_order_contraction(a, b, t_sym, DT_(1.34));
      for(int i(0); i < 3; ++i)
      {
        for(int j(0); j < 3; ++j)
        {
...
TensorOperatorTest<float, unsigned int> tagged_tensor_operator_test_f("TensorOperatorTest<float>", PreferredBackend::generic);
TensorOperatorTest<double, unsigned int> tagged_tensor_operator_test_d("TensorOperatorTest<double>", PreferredBackend::generic);
#ifdef FEAT_HAVE_QUADMATH
TensorOperatorTest<__float128, unsigned int> tagged_tensor_operator_test_f128("TensorOperatorTest<__float128>", PreferredBackend::generic);
