...
def write_mesh_to_file(x0, x1, y0, y1, z0, z1, m, n, l, filename, myself = ""):
  print("Trying to create 3D hexa cube in file " + filename)
  #a few sanity checks:
  if(x0 >= x1):
  ...
  with open(filename, "wt") as f:
    ...
    if(myself):
      ...
      f.write("  <!-- Generated by the FEAT 3d_hexa_cube.py tool -->\n")
      f.write(" <!-- call:  mesh_to_file")

    f.write(" %.3f %.3f %.3f %.3f %.3f %.3f %i %i %i %s" % (x0, x1, y0, y1, z0, z1, m, n, l, filename))
    f.write(" -->\n")
    # write mesh
...
def main():
  ...
  write_mesh_to_file(x0, x1, y0, y1, z0, z1, m, n, l, filename, myself)
