...
namespace VoxelMapGenerator
{
  ...
  int check_voxel_maps(Dist::Comm& comm, SimpleArgParser& args)
  {
    ...
    {
      std::cout << "Reading voxel map file '" << filename << "'..." << std::endl;
      ...
      {
        std::cout << "ERROR: File is too small to even contain the voxel map header!" << std::endl;
        return 1;
      }
      ...
      {
        std::cout << "ERROR: File has incorrect magic number; this does not seem to be a valid voxel map file!" << std::endl;
        return 1;
      }
      ...
      {
        std::cout << "ERROR: File has incorrect header size; maybe this file was created for a newer version?" << std::endl;
        std::cout << "Expected header size: " << Geometry::VoxelMap::header_size << " but found " << header.header_size << std::endl;
        return 1;
      }
      ...
      std::cout << "Total File Size....: " << stringify_bytes(filesize, 3, 12) << std::endl;
      if(compress_size > 0u)
      {
  ...
  int main(int argc, char** argv)
  {
    ...
    {
      std::cerr << "ERROR: failed to parse --render parameter '" << args.get_arg(-iarg) << "' as resolution!" << std::endl;
      display_help(comm, args);
      return 1;
    }
    ...
    else // create voxel map somehow
    {
      ...
      {
        std::cerr << "ERROR: invalid number of arguments for --box parameter: must be either 4 (2D) or 6 (3D)" << std::endl;
        return 1;
      }
      else
      {
        std::cerr << "ERROR: failed to parse --box parameter '" << args.get_arg(-iarg) << "' as coordinate!" << std::endl;
        return 1;
      }
      ...
      if(args.check("num") > 0)
      {
        ...
        if(iarg < dimension)
        {
          std::cerr << "ERROR: failed to parse --num parameter '" << args.get_arg(-iarg) << "' as count!" << std::endl;
          return 1;
        }
      ...
      else if(args.check("res") > 0)
      {
        ...
        if(iarg < 1)
        {
          std::cerr << "ERROR: failed to parse --res parameter '" << args.get_arg(-iarg) << "' as resolution!" << std::endl;
          return 1;
        }
        else if(res < 1e-6)
        {
          std::cerr << "ERROR: invalid resolution '" << res << "'!" << std::endl;
          return 1;
        }
      ...
      {
        std::cerr << "ERROR: either '--num <num-x> <num-y> <num-z>' or '--res <delta>' mus be given!" << std::endl;
        display_help(comm, args);
        return 1;
      }
    }
