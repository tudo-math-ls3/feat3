...
public:
  explicit SpaceTransferTest(PreferredBackend backend) :
    UnitTest("SpaceTransferTest", Type::Traits<DataType_>::name(), Type::Traits<IndexType_>::name(), backend)
  {
  }
  ...
  void test_unit_2d_q2q1() const
  {
    ...
    TEST_CHECK_LESS_THAN(vec_q2t_1.norm2(), eps);

    vec_q2t_2.axpy(vec_q2, -DataType_(1));
    TEST_CHECK_LESS_THAN(vec_q2t_2.norm2(), eps);
    ...
    TEST_CHECK_LESS_THAN(vec_q1t_1.norm2(), eps);

    vec_q1t_2.axpy(vec_q1, -DataType_(1));
    TEST_CHECK_LESS_THAN(vec_q1t_2.norm2(), eps);
  }
};

SPAWN_UNIT_TEST_2T_P(SpaceTransferTest, double, std::uint64_t, PreferredBackend::generic);
#ifdef FEAT_HAVE_MKL
SPAWN_UNIT_TEST_2T_P(SpaceTransferTest, double, std::uint64_t, PreferredBackend::mkl);
#endif
#ifdef FEAT_HAVE_QUADMATH
SPAWN_UNIT_TEST_2T_P(SpaceTransferTest, __float128, std::uint64_t, PreferredBackend::generic);
#endif
#ifdef FEAT_HAVE_HALFMATH
SPAWN_UNIT_TEST_2T_P(SpaceTransferTest, Half, std::uint64_t, PreferredBackend::generic);
#endif
#ifdef FEAT_HAVE_CUDA
SPAWN_UNIT_TEST_2T_P(SpaceTransferTest, float, std::uint32_t, PreferredBackend::cuda);
SPAWN_UNIT_TEST_2T_P(SpaceTransferTest, double, std::uint64_t, PreferredBackend::cuda);
#endif
