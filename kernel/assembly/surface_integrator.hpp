...
namespace FEAT::Assembly
{
   ...
   * We generally assume we view from outside, so you have to set the _view_inside variable if this is not case.
   */
  ...
  private:
    ...
    /// tests whether two boundingbox intersect in any way
  ...
  public:
    ...
    void compile()
    {
      ...
      {
        // reserve 128 elements per entry, this should be enough without triggering a reallocation
  ...
  public:
    ...
    {
      for(IndexType i = 0; i < verts_per_face; ++i)
      {
      ...
      for(IndexType i = 0; i < verts_per_face; ++i)
      {
        _shape_index_tmp.back()[i] = num_indx*verts_per_face + i;
      }
    }
    ...
    void assemble_omp(Job_& job)
    {
      ...
      FEAT_PRAGMA_OMP(parallel)
      {
        ...
        for(IndexType k = 0; k < _active_surfaces.size(); ++k)
        {
          ...
          for(Index pt = 0; pt < Index(_cubature_rule.get_num_points()); ++pt)
          {
