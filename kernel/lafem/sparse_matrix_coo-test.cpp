...
/**
 * \brief Test class for the convertion from SparseMatrixBanded to SparseMatrixCOO
 *
 * \test test description missing
 *
 * \tparam Mem_
...
template<
  typename Mem_,
  typename DT_>
class SparseMatrixCOObandedTest
  : public TaggedTest<Mem_, DT_>
{
public:
  SparseMatrixCOObandedTest()
    : TaggedTest<Mem_, DT_>("SparseMatrixCOObandedTest")
  {
  }
  ...
  virtual void run() const
  {
    ...
    SparseMatrixCOO<Mem_, DT_, Index> sys_coo(sys_banded);

    DenseVector<Mem_, DT_, Index> x_banded(sys_banded.columns(), DT_(0.0));
    DenseVector<Mem_, DT_, Index> y_banded(sys_banded.rows());
    DenseVector<Mem_, DT_, Index> x_coo(sys_coo.columns(), DT_(0.0));
    DenseVector<Mem_, DT_, Index> y_coo(sys_coo.rows());
    ...
    for (Index i(0); i < sys_banded.columns(); ++i)
    {
      ...
      sys_coo.template apply<Algo_>(y_coo, x_coo);

      // check, if the result is correct
      for (Index j(0) ; j < sys_banded.rows() ; ++j)
      {
        TEST_CHECK_EQUAL_WITHIN_EPS(y_banded(j), y_coo(j), 1e-8);
      }
...
SparseMatrixCOObandedTest<Mem::Main, float> cpu_sparse_matrix_coo_banded_test_float;
SparseMatrixCOObandedTest<Mem::Main, double> cpu_sparse_matrix_coo_banded_test_double;
