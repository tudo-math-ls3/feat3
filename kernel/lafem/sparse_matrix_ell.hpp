...
namespace FEAT
{
  namespace LAFEM
  {
    ...
    public:
      ...
      public:
                        ...
                        const ColMapping_& col_map, DT_ alpha = DT_(1))
        {
          ...
          {
            // fetch row index
            const IT_ ix = IT_(row_map.get_index(i));
            ...
            for(int j(0); j < col_map.get_num_local_dofs(); ++j)
            {
              ...
              _val[_col_ptr[jx]] += alpha * loc_mat[i][j];
                        ...
                        const ColMapping_& col_map, DT_ alpha = DT_(1))
        {
          ...
          for(int i(0); i < row_map.get_num_local_dofs(); ++i)
          {
            ...
            for(int j(0); j < col_map.get_num_local_dofs(); ++j)
            {
              ...
              loc_mat[i][j] += alpha * _val[_col_ptr[jx]];
    ...
    public:
      ...
      using ContainerType = SparseMatrixELL<Mem2_, DT2_, IT2_>;
      ...
      void clone(const SparseMatrixELL<Mem2_, DT2_, IT2_> & other, CloneMode clone_mode = CloneMode::Weak)
      {
                ...
                DT_ alpha = DT_(1))
      {
        ...
        XASSERTM(x.C() == y.C(), "Matrix chunk size do not match!");
        XASSERTM(x.C() == this->C(), "Matrix chunk size do not match!");
      ...
      void scale(const SparseMatrixELL & x, const DT_ alpha)
      {
        ...
        XASSERTM(x.C() == this->C(), "Chunk size does not match!");
      ...
      void row_norm2(VectorTypeL& row_norms) const
      {
        ...
        Arch::RowNorm<Mem_>::ell_norm2(row_norms.elements(), this->val(), col_ind(), cs(), cl(), C(), rows());
      ...
      void row_norm2sqr(VectorTypeL& row_norms) const
      {
        ...
        Arch::RowNorm<Mem_>::ell_norm2sqr(row_norms.elements(), this->val(), col_ind(), cs(), cl(), C(), rows());
      ...
      void row_norm2sqr(VectorTypeL& row_norms, const VectorTypeR& scal) const
      {
        ...
        Arch::RowNorm<Mem_>::ell_scaled_norm2sqr(row_norms.elements(), scal.elements(), this->val(), col_ind(), cs(),
        cl(), C(), rows());
      ...
      void transpose(const SparseMatrixELL & x)
      {
        ...
        for (Index i(0); i < txcolumns; ++i)
        {
          for (IT_ k(ptrl[i]); k < ptcl[i/tC]; ++k)
          {
            ptcol_ind[ptcs[i/tC] + i%tC + k*tC] = IT_(0);
            ptval    [ptcs[i/tC] + i%tC + k*tC] = DT_(0);
          }
        }
