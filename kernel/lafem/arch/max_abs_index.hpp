...
#define KERNEL_LAFEM_ARCH_MAX_ABS_INDEX_HPP 1
...
#ifdef FEAT_EICKT
      extern template Index MaxAbsIndex<Mem::Main>::value_generic(const float * const, const Index);
      extern template Index MaxAbsIndex<Mem::Main>::value_generic(const double * const, const Index);
#endif

      template <>
      struct MaxAbsIndex<Mem::CUDA>
      {
        template <typename DT_>
        static Index value(const DT_ * const x, const Index size);
      };
...
#ifndef  __CUDACC__
#include <kernel/lafem/arch/max_abs_index_generic.hpp>
#endif
#endif // KERNEL_LAFEM_ARCH_MAX_ABS_INDEX_HPP
