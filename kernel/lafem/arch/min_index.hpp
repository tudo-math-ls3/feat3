...
#ifdef FEAT_EICKT
      extern template Index MinIndex<Mem::Main>::value_generic(const float * const, const Index);
      extern template Index MinIndex<Mem::Main>::value_generic(const double * const, const Index);
#endif

      template <>
      struct MinIndex<Mem::CUDA>
      {
        template <typename DT_>
