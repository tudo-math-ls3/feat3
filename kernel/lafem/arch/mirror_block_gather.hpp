...
{
  struct MirrorBlockGather
  {
    template<typename DT_, typename IT_>
    static void exec_generic_impl(const Index bs, const Index boff, const Index nidx, const IT_* idx, DT_* buf, const DT_* vec);

    template<typename DT_, typename IT_>
    static void exec_cuda_impl(const Index bs, const Index boff, const Index nidx, const IT_* idx, DT_* buf, const DT_* vec);
    ...
    static void exec_generic(const Index boff, const Index nidx, const Memory::Arbiter& idx, Memory::Arbiter& buf, const Memory::Arbiter& vec)
    {
      ...
      return exec_generic_impl(Index(block_size_), boff, nidx, i_view.get_r(), b_view.get_w(), x_view.get_r());
    }
    ...
    static void exec_cuda(const Index boff, const Index nidx, const Memory::Arbiter& idx, Memory::Arbiter& buf, const Memory::Arbiter& vec)
    {
      ...
      return exec_cuda_impl(Index(block_size_), boff, nidx, i_view.get_r(), b_view.get_w(), x_view.get_r());
    }
    ...
    static void exec(const Index boff, const Index nidx, const Memory::Arbiter& idx, Memory::Arbiter& buf, const Memory::Arbiter& vec)
    {
      ...
      case PreferredBackend::generic:
        exec_generic<DT_, IT_, block_size_>(boff, nidx, idx, buf, vec);
        break;

      case PreferredBackend::mkl:
#ifdef FEAT_HAVE_MKL
        Backend::warn_missing("LAFEM::Arch::MirrorBlockGather::exec");
        exec_generic<DT_, IT_, block_size_>(boff, nidx, idx, buf, vec);
#else
        XABORTM("LAFEM::Arch::MirrorBlockGather::exec: MKL backend not available!");
#endif
        break;

      case PreferredBackend::cuda:
#ifdef FEAT_HAVE_CUDA
        exec_cuda<DT_, IT_, block_size_>(boff, nidx, idx, buf, vec);
#else
        XABORTM("LAFEM::Arch::MirrorBlockGather::exec: CUDA backend not available!");
#endif
        break;

      default:
        XABORTM("LAFEM::Arch::MirrorBlockGather::exec: unknown backend!");
        break;
      }
    }
  }; // struct MirrorBlockGather
...
#ifndef __CUDACC__
#define KERNEL_LAFEM_ARCH_MIRROR_BLOCK_GATHER_HPP 1
#include <kernel/lafem/arch/mirror_block_gather_generic.hpp>
#endif
