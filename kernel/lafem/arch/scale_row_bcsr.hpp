...
#ifdef FEAT_HAVE_MKL
        Backend::warn_missing("LAFEM::Arch::ScaleRowBCSR::exec");
        exec_generic<DT_, IT_>(r, a, col_idx, row_ptr, x, rows, cols, nonzeros, bh, bw);
#else
        XABORTM("LAFEM::Arch::ScaleRowBCSR::exec: MKL backend not available!");
#endif
        break;
...
#else
        XABORTM("LAFEM::Arch::ScaleRowBCSR::exec: CUDA backend not available!");
#endif
        break;

      default:
        XABORTM("LAFEM::Arch::ScaleRowBCSR::exec: unknown backend!");
        break;
      }
    }
  };
...
#ifdef FEAT_EICKT
  extern template void ScaleRowBCSR::exec_generic_impl(float *, const float * const, const std::uint64_t * const, const std::uint64_t * const, const float * const, const Index, const Index, const Index, const int, const int);
  extern template void ScaleRowBCSR::exec_generic_impl(double *, const double * const, const std::uint64_t * const, const std::uint64_t * const, const double * const, const Index, const Index, const Index, const int, const int);
  extern template void ScaleRowBCSR::exec_generic_impl(float *, const float * const, const std::uint32_t * const, const std::uint32_t * const, const float * const, const Index, const Index, const Index, const int, const int);
  extern template void ScaleRowBCSR::exec_generic_impl(double *, const double * const, const std::uint32_t * const, const std::uint32_t * const, const double * const, const Index, const Index, const Index, const int, const int);
#endif
} // namespace FEAT::LAFEM::Arch

#ifndef  __CUDACC__
#define KERNEL_LAFEM_ARCH_SCALE_ROW_BCSR_HPP 1
#include <kernel/lafem/arch/scale_row_bcsr_generic.hpp>
#endif
