...
{
  struct MirrorDenseScatter
  {
    template<typename DT_, typename IT_>
    static void exec_generic_impl(const Index boff, const Index nidx, const IT_* idx, const DT_* buf, DT_* vec, const DT_ alpha);

    template<typename DT_, typename IT_>
    static void exec_cuda_impl(const Index boff, const Index nidx, const IT_* idx, const DT_* buf, DT_* vec, const DT_ alpha);
    ...
    static void exec_generic(const Index boff, const Index nidx, const Memory::Arbiter& idx, const Memory::Arbiter& buf, Memory::Arbiter& vec, const DT_ alpha)
    {
      ...
      return exec_generic_impl(boff, nidx, i_view.get_r(), b_view.get_r(), x_view.get_w(), alpha);
    }
    ...
    static void exec_cuda(const Index boff, const Index nidx, const Memory::Arbiter& idx, const Memory::Arbiter& buf, Memory::Arbiter& vec, const DT_ alpha)
    {
      ...
      return exec_cuda_impl(boff, nidx, i_view.get_r(), b_view.get_r(), x_view.get_w(), alpha);
    }
    ...
    static void exec(const Index boff, const Index nidx, const Memory::Arbiter& idx, const Memory::Arbiter& buf, Memory::Arbiter& vec, const DT_ alpha)
    {
      ...
      case PreferredBackend::generic:
        exec_generic<DT_, IT_>(boff, nidx, idx, buf, vec, alpha);
        break;

      case PreferredBackend::mkl:
#ifdef FEAT_HAVE_MKL
        Backend::warn_missing("LAFEM::Arch::MirrorDenseScatter::exec");
        exec_generic<DT_, IT_>(boff, nidx, idx, buf, vec, alpha);
#else
        XABORTM("LAFEM::Arch::MirrorDenseScatter::exec: MKL backend not available!");
#endif
        break;

      case PreferredBackend::cuda:
#ifdef FEAT_HAVE_CUDA
        exec_cuda<DT_, IT_>(boff, nidx, idx, buf, vec, alpha);
#else
        XABORTM("LAFEM::Arch::MirrorDenseScatter::exec: CUDA backend not available!");
#endif
        break;

      default:
        XABORTM("LAFEM::Arch::MirrorDenseScatter::exec: unknown backend!");
        break;
      }
    }
  }; // struct MirrorDenseScatter
...
#ifndef __CUDACC__
#define KERNEL_LAFEM_ARCH_MIRROR_DENSE_SCATTER_HPP 1
#include <kernel/lafem/arch/mirror_dense_scatter_generic.hpp>
#endif
