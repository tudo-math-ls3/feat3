  ...
  struct MaxRelDiffDense
  {
    ...
    static DT_ exec(const Memory::Arbiter& x, const Memory::Arbiter& y, const Index size)
    {
...
#ifdef FEAT_HAVE_MKL
        //Backend::warn_missing("LAFEM::Arch::MaxRelDiffDense::exec");
        return exec_generic<DT_>(x, y, size);
#else
        XABORTM("LAFEM::Arch::MaxRelDiffDense::exec: MKL backend not available!");
#endif
        break;
      ...
      case PreferredBackend::cuda:
        ...
        return exec_generic<DT_>(x, y, size);
#else
        XABORTM("LAFEM::Arch::MaxRelDiffDense::exec: CUDA backend not available!");
#endif
        break;

      default:
        XABORTM("LAFEM::Arch::MaxRelDiffDense::exec: unknown backend!");
        break;
      }
    }
  };
...
#ifdef FEAT_EICKT
  extern template float MaxRelDiffDense::exec_generic_impl(const float * const, const float * const, const Index);
  extern template double MaxRelDiffDense::exec_generic_impl(const double * const, const double * const, const Index);
#endif

} // namespace FEAT::LAFEM::Arch

#ifndef  __CUDACC__
#define KERNEL_LAFEM_ARCH_MAX_REL_DIFF_DENSE_HPP 1
#include <kernel/lafem/arch/max_rel_diff_dense_generic.hpp>
#endif
