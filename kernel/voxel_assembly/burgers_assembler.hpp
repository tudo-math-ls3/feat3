...
namespace FEAT
{
  namespace VoxelAssembly
  {
    ...
    public:
      ...
      {
        if(Backend::get_preferred_backend() == PreferredBackend::cuda)
          set_sd_v_norm_cuda(convect);
        else
          set_sd_v_norm_generic(convect);
      }
       ...
       const SpaceType& space, typename Cubature::Rule<ShapeType, DataType, DataType>::PointType* cub_pt, const DataType* cub_wg, int num_cubs, DataType alpha) const
      {
        ...
        Memory::TypedView<DataType> val_view(matrix.val_view_raw_rw());
        ...
        const Memory::TypedView<DataType> vec_view = convect.elements_view_raw_r();
        const DataType* vec_data = vec_view.get_r();
        ...
        VoxelAssembly::Arch::assemble_burgers_csr_host(space, mat_data, vec_data, cub_data, mapping_data, mesh_data.coloring_data, alpha, burgers_params);
      }
       ...
       const SpaceType& space, typename Cubature::Rule<ShapeType, DataType, DataType>::PointType* cub_pt, const DataType* cub_wg, int num_cubs, DataType alpha) const
      {
        ...
        Memory::TypedView<DataType> vec_view = vector.elements_view_raw_rw();
        const Memory::TypedView<DataType> conv_view = convect.elements_view_raw_r();
        const Memory::TypedView<DataType> primal_view = primal.elements_view_raw_r();
        DataType* vec_data = vec_view.get_w();
        const DataType* conv_data = conv_view.get_r();
        const DataType* primal_data = primal_view.get_r();
        ...
        VoxelAssembly::Arch::assemble_burgers_defect_host(space, vec_data, conv_data, primal_data, cub_data, mapping_data, mesh_data.coloring_data, alpha, burgers_params);
        //free resources
      }
       ...
       const SpaceType& space, typename Cubature::Rule<ShapeType, DataType, DataType>::PointType* cub_pt, const DataType* cub_wg, int num_cubs, DataType alpha) const
      {
        //VoxelAssembly::CSRMatrixData<DataType, IndexType> mat_data = {matrix.template val<LAFEM::Perspective::pod>(), matrix.row_ptr(), matrix.col_idx(), matrix.num_rows(), matrix.num_cols()};
        //const DataType* vec_data = convect.template elements<LAFEM::Perspective::pod>();
        Memory::TypedView<DataType> val_view(matrix.val_view_raw_rw(Memory::Location::cuda));
        ...
        const Memory::TypedView<DataType> vec_view = convect.elements_view_raw_r(Memory::Location::cuda);
        const DataType* vec_data = vec_view.get_r();
        ...
        VoxelAssembly::Arch::assemble_burgers_csr(space, mat_data, vec_data, d_cub_data,
                                                        d_mapping_data, mesh_data.coloring_data, alpha, burgers_params,
       ...
       const SpaceType& space, typename Cubature::Rule<ShapeType, DataType, DataType>::PointType* cub_pt, const DataType* cub_wg, int num_cubs, DataType alpha) const
      {
        ...
        Memory::TypedView<DataType> vec_view = vector.elements_view_raw_rw(Memory::Location::cuda);
        const Memory::TypedView<DataType> conv_view = convect.elements_view_raw_r(Memory::Location::cuda);
        const Memory::TypedView<DataType> primal_view = primal.elements_view_raw_r(Memory::Location::cuda);
        DataType* vec_data = vec_view.get_w();
        const DataType* conv_data = conv_view.get_r();
        const DataType* primal_data = primal_view.get_r();
        ...
        VoxelAssembly::Arch::assemble_burgers_defect(space, vec_data, conv_data, primal_data, d_cub_data, d_mapping_data,
                                                              mesh_data.coloring_data, alpha, burgers_params,
      ...
      void set_kernel_launch_params(int target_elements, int blocksize_)
      {
        ...
        // TODO: this only works for simple burgers...
        ...
        const int max_color_sizes = int(mesh_data.coloring_data.get_max_color_size());
      ...
      #else //FEAT_HAVE_CUDA
      void assemble_matrix1_cuda(LAFEM::SparseMatrixBCSR<DataType, IndexType, dim, dim>& DOXY(matrix), const LAFEM::DenseVectorBlocked<DataType, IndexType, dim>&, const SpaceType& DOXY(space), typename Cubature::Rule<ShapeType, DataType, DataType>::PointType* DOXY(cub_pt), const DataType* DOXY(cub_wg), int DOXY(num_cubs), DataType DOXY(alpha)) const
      {
