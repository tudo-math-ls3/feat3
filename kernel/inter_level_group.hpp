...
namespace FEAST
{
  ...
  *
  * Example (see sketch above): C and E compute their local portions of the global defect to A. A assembles these two
  * portions and computes a correction vector. The two parts of this correction vector corresponding to the lower left
  * and upper left subregion, resp., are then sent back to C and E.
  *
  * We require that a region, which is scheduled to one process on grid level L is also scheduled to exactly one process on
  * all coarser levels l<L. On a finer level l>L, however, the region may be scheduled to more than one process.
  *
  ...
  * On the other hand, defects can be received from several processes (where one of these processes may be the same as the
  * receiving one) and corrections can be sent to several processes.
  *
  * All MPI communication is done via extra communicators containing only the processes of the different work groups that
  * have to exchange data with each other. In the example above, these are:
  *   communicator 1:  0 (CG-WG + FG-WG), 3 (FG-WG)
  *   communicator 2:  1 (CG-WG), 2, 4 (FG-WG)
  * This way, all necessary data exchange can be performed efficiently via corresponding collective communication routines
  * (e.g., receive portions of the defect vector via MPI_Gather and send portions of the correction vector via MPI_Scatter,
  * where the CG-WG process is the root, resp.).
  ...
  * COMMENT_HILMAR: Oder vielleicht doch? Collective Routinen können ja auch 'sich selbst' schreiben...
  * Mal sehen...
  *
