#pragma once
#ifndef KERNEL_MESHOPT_RUMPF_FUNCTIONALS_LVLSET_MONITOR_2D_Q1_D1_HPP
#define KERNEL_MESHOPT_RUMPF_FUNCTIONALS_LVLSET_MONITOR_2D_Q1_D1_HPP 1
...
namespace FEAST
{
  namespace Meshopt
  {
    ...
    class RumpfFunctionalLevelsetMonitor < MemoryType_, DataType_, Shape::Hypercube<2>> :
    public RumpfFunctionalBase<MemoryType_, DataType_>
    {
      public:
        ...
        typedef FEAST::Tiny::Vector<DataType, 2> MonitorGradType;
      ...
      private:
        static constexpr DataType _mu_1 = DataType(1e-2);
        static constexpr DataType _mu_2 = DataType(1e2);
      ...
      public:
        ...
        void prepare_lvlset(const Tl_& lvlset_vals, const Tlg_& lvlset_grad_vals)
        {
          ...
          for(int i(0); i < Tlg_::n; ++i)
            tmp_grad += lvlset_grad_vals(d,i);

          tmp_grad /= DataType(Tlg_::m);

          _monitor_grad = compute_monitor_der(tmp) * tmp_grad;
          }
          ...
          DataType& lvlset_constraint_last)
          {
            ...
            for(int i(0); i < 4; ++i)
            {
              ...
              auto heaviside_der = FEAST::Assembly::Common::template HeavisideRegStatic<DataType>::der_x(lvlset_prod);
                grad[i] -= heaviside_reg_fac() * fac_lvlset * lvlset_constraint_last *
                  (heaviside_der * lvlset_vals(j) * lvlset_grad_vals[i]);
            }
        ...
        DataType& func_rec_det)
        {
          ...
          DataType tmp = func_norm + func_det + func_rec_det;

          func_norm= this->_monitor;
          func_det = this->_monitor_grad(0);
          func_rec_det = this->_monitor_grad(1);
