    ...
    struct Refinement
    {
                          ...
                          os_<Attribute<DataType_, os_>, std::allocator<Attribute<DataType_, os_> > >& origin_coords)
      {
        ...
        for(Index i(0) ; i < num_faces; i++)
        {
          ...
          for(Index l(0); l < vertex_at_polygon.size()  ; ++l)
          {
            c_x += origin_coords.at(0).at(vertex_at_polygon.at(l));
            c_y += origin_coords.at(1).at(vertex_at_polygon.at(l));
            c_z += origin_coords.at(2).at(vertex_at_polygon.at(l));
          }
          origin_coords.at(0).push_back(c_x/double(vertex_at_polygon.size()));
          origin_coords.at(1).push_back(c_y/double(vertex_at_polygon.size()));
          origin_coords.at(2).push_back(c_z/double(vertex_at_polygon.size()));
        }
