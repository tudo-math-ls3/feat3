...
namespace FEAT
{
  ...
  {
    /**
     * \brief Global base-mesh vector splitter implementation
     *
     * \tparam LocalVector_
    ...
    public:
       ...
       *
       * \note
       * Calling this function is only required on the root process and can be skipped on all other processes.
       *
      ...
      void split(Global::Vector<LocalVector_, Mirror_>& vector, const LocalVector_& vec_base) const
      {
        ...
        if(_muxer.is_parent())
        {
          _muxer.split(vector.local(), vec_base);
        }
        else
        {
          XASSERT(_muxer.is_child());
          _muxer.split_recv(vector.local());
        }
      }
       ...
       *
       * \param[in] filename
       * The filename of the output file that is to be read in.
       *
       * \param[in] mode
