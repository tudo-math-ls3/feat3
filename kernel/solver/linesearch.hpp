  ...
  namespace Solver
  {
    ...
    class NewtonRaphsonLinesearch : public Linesearch<Operator_, Filter_>
    {
      ...
      protected:
        ...
        virtual Status _apply_intern(VectorType& sol, const VectorType& dir)
        {
          ...
          //DataType alpha(DataType(1)/this->_norm_dir);
          DataType alpha(0);
          ...
          while(status == Status::progress)
          {
            ...
            DataType fval = this->_op.compute_func();
              std::cout << "fval = " << stringify_fp_sci(fval) << " fval_min = " << stringify_fp_sci(this->_fval_min)
              << " alpha = " << stringify_fp_sci(alpha) << " alpha_min = " << stringify_fp_sci(this->_alpha_min) <<
              std::endl;
            ...
            alpha += alpha_update;
            std::cout << "alpha_update = " << stringify_fp_sci(alpha_update) << std::endl;
    ...
    class SecantLinesearch : public Linesearch<Operator_, Filter_>
    {
      ...
      public:
        ...
        /// \copydoc NewtonRaphsonLinesearch::get_rel_update()
        DataType get_rel_update()
        {
      ...
      protected:
        ...
        virtual Status _apply_intern(VectorType& sol, const VectorType& dir)
        {
          ...
          std::cout << "SecantLinesearch: fval_0 = " << this->_fval_min << std::endl;
          ...
          while(status == Status::progress)
          {
            ...
            std::cout << "SLS " << this->_num_iter << " sol = " << sol << " grad = " << this->_vec_grad << " dir = " << dir << std::endl;
            std::cout << "    alpha = " << stringify_fp_sci(alpha) << " alpha_update = " << stringify_fp_sci(alpha_update) << " fval = " << stringify_fp_sci(fval) << std::endl;
    ...
    class StrongWolfeLinesearch : public Linesearch<Operator_, Filter_>
    {
      ...
      protected:
        ...
        virtual Status _apply_intern(VectorType& vec_sol, const VectorType& vec_dir)
        {
          ...
          // Set the soft limits. These are important if we know that the minimum is NOT in the current intervall of
          ...
          while(status == Status::progress)
          {
            ...
            // Stop if _bracket was successfull or encountered an error
            if(status != Status::progress)
              return status;
            ...
            // If the curvature condition is satisfied, the current alpha is good and we are successfull
            ...
            // If we come to here, we do not know if the minimum is in the current intervall of uncertainity, so we
            ...
            _polynomial_fit(alpha_new, alpha, alpha_prev, fval, fval_prev, delta, delta_prev, min_in_intervall,
            (delta < 0));
        ...
        Status _bracket(VectorType& vec_sol, const VectorType& vec_dir, DataType& alpha_lo, DataType& alpha_hi, DataType& delta_lo, DataType& delta_hi, DataType& f_lo, DataType& f_hi)
        {
          ...
          while(st == Status::progress && iter < 100)
          {
            ...
            // Update of intervall of uncertainity. We might already be successfull.
            ...
            else
            {
              ...
              // If the curvature condition is satisfied too, we are successfull
          ...
          // If we have not been successfull, use the best step size obtained so far.
        ...
        Status _polynomial_fit(DataType& alpha_new, DataType alpha_lo, DataType alpha, DataType f_lo, DataType f, DataType df_lo, DataType df, bool& min_in_intervall, bool df_was_negative) const
        {
