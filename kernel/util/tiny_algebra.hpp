  ...
  namespace Tiny
  {
    ...
    public:
      ...
      CUDA_HOST_DEVICE Matrix& set_cofactor(const Matrix<T_, m_, n_, sma_, sna_>& a)
      {
        ...
        typedef DataType Tv[sm_][sn_];
        typedef DataType Ta[sma_][sna_];
        Intern::CofHelper<m_,n_>::template compute<DataType, sm_, sn_, sma_, sna_>(
          *reinterpret_cast<Tv*>(this), *reinterpret_cast<const Ta*>(&a));
        return *this;
      }
    ...
    namespace Intern
    {
      ...
      struct CofHelper
      {
        ...
        CUDA_HOST_DEVICE static void compute(T_ (&b)[smb_][snb_], const T_ (&a)[sma_][sna_])
        {
          // Note: We need to specify the template arguments for the 'compute' function explicitly as the
          //       Intel C++ compiler cannot deduct the arguments automatically due to a compiler bug.
          typedef T_ Tva[sma_][sna_];
          T_ fac = Intern::DetHelper<m_, n_>::template
            compute<T_, sma_, sna_>(*reinterpret_cast<const Tva*>(a));

          // Note: We need to specify the template arguments for the 'compute' function explicitly as the
          //       Intel C++ compiler cannot deduct the arguments automatically due to a compiler bug.
          typedef T_ Tvb[smb_][snb_];

          Tvb b_tmp;
          Intern::InverseHelper<m_,n_>::template compute<T_, smb_, snb_, sma_, sna_>(
            *reinterpret_cast<Tvb*>(b_tmp), *reinterpret_cast<const Tva*>(&a));
      ...
      struct VolHelper
      {
        ...
        CUDA_HOST_DEVICE static T_ compute(const T_ (&a)[sm_][sn_])
        {
          ...
          #else
          return CudaMath::cuda_sqrt(DetHelper<n_,n_>::template compute<T_,n_,n_>(b));
          #endif
        }
      };
      ...
      struct VolHelper<n_,n_>
      {
        ...
        CUDA_HOST_DEVICE static T_ compute(const T_ (&a)[sm_][sn_])
        {
          ...
          #else
          return CudaMath::cuda_abs(DetHelper<n_,n_>::template compute<T_,sm_,sn_>(a));
          #endif
        }
      };
      ...
      struct InverseHelper<n_, n_>
      {
        ...
        CUDA_HOST_DEVICE static void compute(T_ (&b)[smb_][snb_], const T_ (&a)[sma_][sna_])
        {
          ...
          #else
          CudaMath::cuda_invert_matrix(n_, snb_, &b[0][0], p);
          #endif
        }
      };
    } // namespace Intern
