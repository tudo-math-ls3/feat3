...
namespace ClusterMultigridBench
{
  ...
  void main(int argc, char** argv, const double init_time)
  {
    ...
    for(Index ilv = 0; ilv < solve_levels; ++ilv)
    {
      ...
      auto multigrid = Solver::new_multigrid(multigrid_hierarchy, Solver::MultiGridCycle::V, int(ilv));
      ...
      for(Index i(0); i+ilv < multigrid_hierarchy->size_physical(); ++i)
      {
        mg_v_stats.times.at(i)[SolverTimes::gmg_defect] = multigrid_hierarchy->get_time_defect(int(ilv+i));
        mg_v_stats.times.at(i)[SolverTimes::gmg_smooth] = multigrid_hierarchy->get_time_smooth(int(ilv+i));
        mg_v_stats.times.at(i)[SolverTimes::gmg_transfer] = multigrid_hierarchy->get_time_transfer(int(ilv+i));
        mg_v_stats.times.at(i)[SolverTimes::gmg_coarse] = multigrid_hierarchy->get_time_coarse(int(ilv+i));
      ...
      for(Index i(0); i+ilv < multigrid_hierarchy->size_physical(); ++i)
      {
        mg_f_stats.times.at(i)[SolverTimes::gmg_defect] = multigrid_hierarchy->get_time_defect(int(ilv+i));
        mg_f_stats.times.at(i)[SolverTimes::gmg_smooth] = multigrid_hierarchy->get_time_smooth(int(ilv+i));
        mg_f_stats.times.at(i)[SolverTimes::gmg_transfer] = multigrid_hierarchy->get_time_transfer(int(ilv+i));
        mg_f_stats.times.at(i)[SolverTimes::gmg_coarse] = multigrid_hierarchy->get_time_coarse(int(ilv+i));
    ...
    for(Index i = 0; i < solve_levels; ++i)
    {
