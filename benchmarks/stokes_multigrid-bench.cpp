...
namespace StokesMultigridBench
{
  ...
  struct Counts
  {
    ...
    static constexpr std::size_t num_dofs_l_v = 3u; // local velocity dofs
    static constexpr std::size_t num_dofs_l_p = 4u; // local pressure dofs
    static constexpr std::size_t num_dofs_g = 5u; // global dofs [ != sum(num_dofs_l) ]
    static constexpr std::size_t num_dofs_g_v = 6u;
    static constexpr std::size_t num_dofs_g_p = 7u;
    static constexpr std::size_t num_nze = 8u;
    static constexpr std::size_t bytes_domain = 9u;
    static constexpr std::size_t bytes_system = 10u;
    static constexpr std::size_t bytes_solver = 11u;
    static constexpr std::size_t elems_mirror = 12u;
  ...
  struct BenchStats
  {
    ...
    // final relative defect and final errors
    double final_rel_def;
      ...
      final_rel_def(0.0)
    {
      ...
      final_errors[0u] = final_errors[1u] = 0.0;
    }
    ...
    String format() const
    {
      ...
      s += "\nFinal Normalized Errors....: " + stringify_fp_sci(final_errors[0u]) + " " + stringify_fp_sci(final_errors[1u]);
  ...
  void main(int argc, char** argv)
  {
    ...
    {
      comm.print(
        "FEAT3 2D-Q2 Stokes-Multigrid Solver Benchmark\n"
        "----------------------------------------------\n"
        "This is a small benchmark application that will create a rectilinear 2D grid, then partition and\n"
        "distribute it to an arbitrary number of MPI processes to solve a Stokes PDE discretized by a\n"
        "conforming Q2/P1dc finite element scheme and (approximately) solve it by a geometric multigrid\n"
    ...
    // choose problem size based on memory requirement?
    ...
    else if(args.check("memory") > 0)
    {
      ...
      const double bytes_per_elem = 20000.0;
    ...
    typedef Control::StokesBlockedUnitVeloMeanPresSystemLevel<dim, DataType, IndexType> SystemLevelType;
    ...
    const String cubature("gauss-legendre:" + stringify(SpaceVeloType::local_degree+1));
    ...
    for (Index i(0); i < num_levels; ++i)
    {
      ...
      system.at(i)->assemble_gates(domain.at(i));
      ...
      if((i+1) < domain.size_virtual())
      {
        ...
        TimeStamp ts3;
        system.at(i)->assemble_transfers(domain.at(i), domain.at(i+1), cubature);

        if(!no_shrink)
        {
          system.at(i)->transfer_velo.get_mat_prol().shrink(1E-3);
          system.at(i)->transfer_velo.get_mat_rest().shrink(1E-3);
          system.at(i)->transfer_pres.get_mat_prol().shrink(1E-3);
          system.at(i)->transfer_pres.get_mat_rest().shrink(1E-3);
          system.at(i)->compile_system_transfer();
        }
    ...
    Assembly::Common::DuDvOperatorBlocked<dim> dudv_operator;

    for (Index i(0); i < num_levels; ++i)
    {
      TimeStamp ts;
      system.at(i)->assemble_grad_div_matrices(domain.at(i)->domain_asm, domain.at(i)->space_velo, domain.at(i)->space_pres, cubature);
      system.at(i)->assemble_velo_struct(domain.at(i)->space_velo);
      system.at(i)->matrix_a.local().format();
      Assembly::assemble_bilinear_operator_matrix_1(domain.at(i)->domain_asm, system.at(i)->matrix_a.local(), dudv_operator,
    ...
    for (Index i(0); i < num_levels; ++i)
    {
      ...
      auto* mesh_part = domain.at(i)->get_mesh_node()->find_mesh_part("bnd");
      if(mesh_part != nullptr)
        unit_asm.add_mesh_part(*mesh_part);

      unit_asm.assemble(system.at(i)->filter_velo.local(), domain.at(i)->space_velo, taylor_green_velo);

      // assemble pressure mean filter
      system.at(i)->assemble_pressure_mean_filter(domain.at(i)->space_pres, cubature);
      ...
      stats.counts[i][Counts::num_dofs_g_v] = system.at(i)->gate_velo.get_num_global_dofs();
      stats.counts[i][Counts::num_dofs_g_p] = system.at(i)->gate_pres.get_num_global_dofs();
      stats.counts[i][Counts::num_dofs_g] = stats.counts[i][Counts::num_dofs_g_v] + stats.counts[i][Counts::num_dofs_g_p];
      ...
      for (auto& mirror : sys_mirrors)
      {
        ...
        stats.counts[i][Counts::elems_mirror] += mirror.template at<1>().num_indices();
      }
    }
    ...
    std::deque<LocalSystemMatrix> system_matrices_type1(num_levels);

    for (Index i(0); i < num_levels; ++i)
    {
      TimeStamp ts;

      // convert to type 1 matrices
      system_matrices_type1.at(i).block_a() = system.at(i)->matrix_a.convert_to_1();
      system_matrices_type1.at(i).block_b() = system.at(i)->matrix_b.local().clone(LAFEM::CloneMode::Weak);
      system_matrices_type1.at(i).block_d() = system.at(i)->matrix_d.local().clone(LAFEM::CloneMode::Shallow);

      // filter local type-1 matrices
      system.at(i)->filter_velo.local().filter_mat(system_matrices_type1.at(i).block_a());
    ...
    // interpolate Taylor-Green velocity and pressure functions
    Assembly::Interpolator::project(vec_ref.local().template at<0>(), taylor_green_velo, the_domain_level.space_velo);
    Assembly::Interpolator::project(vec_ref.local().template at<1>(), taylor_green_pres, the_domain_level.space_pres);
    ...
    for (Index i(0); i < num_levels; ++i)
    {
      ...
      if((i+1) < domain.size_virtual())
      {
        auto vanka = Solver::new_amavanka(system_matrices_type1.at(i), lvl.filter_sys.local());
        ...
        // Multigrid: 5 Vectors
        stats.counts.at(i)[Counts::bytes_solver] += 7ull * lvl_dofs * sizeof(DataType);
      }
      else
      {
        auto vanka = Solver::new_amavanka(system_matrices_type1.at(i), lvl.filter_sys.local());
        ...
        ama_vankas.push_back(vanka);
        auto schwarz = Solver::new_schwarz_precond(vanka, lvl.filter_sys);
        multigrid_hierarchy->push_level(lvl.matrix_sys, lvl.filter_sys, schwarz);
      }
    }
    ...
    for(Index i(0); i < num_levels; ++i)
    {
      stats.counts.at(i)[Counts::bytes_solver] += ama_vankas.at(i)->bytes();
    }
    ...
    // benchmarking solve
    Solver::Status result = solver->correct(vec_sol, vec_rhs);
    ...
    // compute and save final relative errors
    {
      ...
      stats.final_errors[0u] = Math::sqrt(stats.final_errors[0u] / double(stats.counts.front()[Counts::num_dofs_g_v]));
      stats.final_errors[1u] = Math::sqrt(stats.final_errors[1u] / double(stats.counts.front()[Counts::num_dofs_g_p]));
    }

    /*{
      Geometry::ExportVTK<MeshType> vtk(the_domain_level.get_mesh());
      vtk.add_vertex_vector("v", vec_sol.local().first());
      vtk.write("stokes", comm);
    }*/
    ...
    if(args.check("test") >= 0)
    {
      ...
      bool berrv = (stats.final_errors[0] < 1E-8);
      bool berrp = (stats.final_errors[1] < 1E-7);
      comm.print(String("\nTEST-MODE: CHECK FINAL RELATIVE DEFECT: ") + (bdef ? "OK" : "FAILED"));
      comm.print(String("TEST-MODE: CHECK FINAL VELOCITY ERROR:  ") + (berrv ? "OK" : "FAILED"));
      comm.print(String("TEST-MODE: CHECK FINAL PRESSURE ERROR:  ") + (berrp ? "OK" : "FAILED"));
      comm.print((bdef && berrv && berrp) ? "\nTEST PASSED" : "\nTEST FAILED");
    }
  }
} // namespace StokesMultigridBench
...
int main(int argc, char** argv)
{
  ...
  StokesMultigridBench::main(argc, argv);
  return 0;
}
