...
#include <kernel/geometry/mesh_smoother/laplace_smoother.hpp>
...
//    typedef LaplaceSmoother<TrafoType, DataType, MemType> SmootherType;
    ...
    for(Index i(0); i < boundary.get_num_entities(0); ++i)
    {
      ...
      DataType tmp0 = mr_laplace._coords[0](j);
      DataType tmp1 = mr_laplace._coords[1](j);
      DataType norm = Math::sqrt(DataType(0.5)) / Math::sqrt( Math::sqr(tmp0 - DataType(0.5)) + Math::sqr(tmp1 - DataType(0.5)) );
      mr_laplace._coords[0](j, (tmp0 - DataType(0.5)) * norm );
      mr_laplace._coords[1](j, (tmp1 - DataType(0.5)) * norm );
      //mr_laplace._coords[0](j, (tmp0 - DataType(0.5)) * norm - Math::sqrt(DataType(0.5)));
      //mr_laplace._coords[1](j, (tmp1 - DataType(0.5)) * norm - Math::sqrt(DataType(0.5)));
      //mr_laplace._coords[0](j, tmp0 - ( Math::sin(DataType(2)*pi*tmp1) )/DataType(1 << (level+2)));
      //mr_laplace._coords[1](j, tmp1 + ( Math::sin(DataType(2)*pi*tmp0) )/DataType(1 << (level+2)));
    }

    // Set coords since we used the boundary deformation
    // mr_laplace.set_coords();
    ...
    while(time < DataType(0.2))
    {
      ...
      for(Index i(0); i < boundary.get_num_entities(0); ++i)
      {
        ...
        //mr_laplace._coords[0](j, tmp0 + deltat*tmp1*norm );
        //mr_laplace._coords[1](j, tmp1 - deltat*tmp0*norm );
        mr_laplace._coords[0](j, tmp0 + deltat*DataType(0.25)*((DataType(4)*tmp0 - DataType(2)) + Math::pow((DataType(4)*tmp1 - DataType(2)),DataType(3) ) )) ;
        mr_laplace._coords[1](j, tmp1 - deltat*DataType(0.25)*((DataType(4)*tmp1 - DataType(2)) + Math::pow((DataType(4)*tmp0 - DataType(2)),DataType(3) ) )) ;
        //mr_laplace.set_coords();
