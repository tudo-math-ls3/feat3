...
#  define Q_(x) x
  ...
  // Gauss cubature factory; this is used for integration here,
  // because the coefficients have more that 48 digits precision
  template<int n_>
  class GaussCubatureFactory
  {
  public:
    static_assert((n_ > 0) && (n_ < 6), "invalid n_");

    template<typename C_, typename W_>
    void init(C_ c[], W_ w[]) const
    {
      switch(n_)
      {
      case 1:
      ...
      case 2:
        c[0] = C_(Q_(-0.57735026918962576450914878050195745564760175127));
        c[1] = C_(Q_(+0.57735026918962576450914878050195745564760175127));
        w[0] = W_(Q_(1.0));
        w[1] = W_(Q_(1.0));
        break;
      ...
      case 3:
        c[0] = C_(Q_(+0.0));
        c[1] = C_(Q_(-0.77459666924148337703585307995647992216658434106));
        c[2] = C_(Q_(+0.77459666924148337703585307995647992216658434106));
        w[0] = W_(Q_(0.888888888888888888888888888888888888888888888888));
        w[1] = W_(Q_(0.555555555555555555555555555555555555555555555556));
        w[2] = W_(Q_(0.555555555555555555555555555555555555555555555556));
        break;

      case 4:
        c[0] = C_(Q_(-0.86113631159405257522394648889280950509572537962));
        c[1] = C_(Q_(-0.33998104358485626480266575910324468720057586977));
        c[2] = C_(Q_(+0.33998104358485626480266575910324468720057586977));
        c[3] = C_(Q_(+0.86113631159405257522394648889280950509572537962));
        w[0] = W_(Q_(0.347854845137453857373063949221999407235348695833));
        w[1] = W_(Q_(0.652145154862546142626936050778000592764651304166));
        w[2] = W_(Q_(0.652145154862546142626936050778000592764651304166));
        w[3] = W_(Q_(0.347854845137453857373063949221999407235348695833));
        break;

      case 5:
        c[0] = C_(Q_(+0.0));
        c[1] = C_(Q_(-0.53846931010568309103631442070020880496728660690));
        c[2] = C_(Q_(+0.53846931010568309103631442070020880496728660690));
        c[3] = C_(Q_(-0.90617984593866399279762687829939296512565191076));
        c[4] = C_(Q_(+0.90617984593866399279762687829939296512565191076));
        w[0] = W_(Q_(0.568888888888888888888888888888888888888888888888));
        w[1] = W_(Q_(0.478628670499366468041291514835638192912295553343));
        w[2] = W_(Q_(0.478628670499366468041291514835638192912295553343));
        w[3] = W_(Q_(0.236926885056189087514264040719917362643260002212));
        w[4] = W_(Q_(0.236926885056189087514264040719917362643260002212));
        break;
      }
    }

    template<typename Shape_, typename Weight_, typename Coord_, typename Point_>
    void create_throw(Cubature::Rule<Shape_, Weight_, Coord_, Point_>& rule) const
    {
      if(!create(rule))
        throw Cubature::UnknownRule("GaussCubatureFactory", Shape_::name());
    }

    template<typename Weight_, typename Coord_, typename Point_>
    bool create(Cubature::Rule<Shape::Hypercube<1>, Weight_, Coord_, Point_>& rule) const
    {
      Coord_ c[n_];
      Weight_ w[n_];
      init(c, w);

      rule = Cubature::Rule<Shape::Hypercube<1>, Weight_, Coord_, Point_>(n_, "G5CubatureFactory");
      for(int i(0); i < n_; ++i)
      {
        rule.get_coord(i, 0) = c[i];
        rule.get_weight(i) = w[i];
      }
      return true;
    }

    template<typename Weight_, typename Coord_, typename Point_>
    bool create(Cubature::Rule<Shape::Hypercube<2>, Weight_, Coord_, Point_>& rule) const
    {
      Coord_ c[n_];
      Weight_ w[n_];
      init(c, w);

      rule = Cubature::Rule<Shape::Hypercube<2>, Weight_, Coord_, Point_>(n_*n_, "G5CubatureFactory");
      for(int i(0), l(0); i < n_; ++i)
      {
        for(int j(0); j < n_; ++j, ++l)
        {
          rule.get_coord(l, 0) = c[i];
          rule.get_coord(l, 1) = c[j];
          rule.get_weight(l) = w[i] * w[j];
        }
      }
      return true;
    }
    ...
    template<typename Weight_, typename Coord_, typename Point_>
    bool create(Cubature::Rule<Shape::Hypercube<3>, Weight_, Coord_, Point_>& rule) const
    {
      Coord_ c[n_];
      Weight_ w[n_];
      init(c, w);

      rule = Cubature::Rule<Shape::Hypercube<3>, Weight_, Coord_, Point_>(n_*n_*n_, "G5CubatureFactory");
      for(int i(0), l(0); i < n_; ++i)
      {
        for(int j(0); j < n_; ++j)
        {
          for(int k(0); k < n_; ++k, ++l)
          {
            rule.get_coord(l, 0) = c[i];
            rule.get_coord(l, 1) = c[j];
            rule.get_coord(l, 2) = c[k];
            rule.get_weight(l) = w[i] * w[j] * w[k];
          }
        }
      }
      return true;
