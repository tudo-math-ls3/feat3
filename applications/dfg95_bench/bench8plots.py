...
########################################################################################################################
# DFG95 flow-around-a-sphere-inside-a-cylinder benchmark 8 post-processing script
# -------------------------------------------------------------------------------
#
# USAGE: bench8plots.py <T_min> <outprefix> <logfiles...>
#
# This script analyses a (non-empty) sequence of output logfiles generated by the dfg95-ccnd-unsteady application
# used to solve the 'bench 8' configuration by computing various quantities required by the benchmark setup, e.g.
...
#
# > bench8plots.py 20 k=400_levels lvl=3_k=400.log lvl=4_k=400.log lvl=5_k=400.log lvl=6_k=400.log
#
# would analyse the convergence for various mesh refinement levels (3,4,5,6) for the same time step size k=1/400,
# whereas the call
#
# > bench8plots.py 20 lvl=5_tsteps lvl=5_k=100.log lvl=5_k=200.log lvl=5_k=400.log lvl=5_k=800.log
#
...
if len(sys.argv) < 4:
  print("USAGE: bench8plots.py <T_min> <outprefix> <logfiles...>")
  sys.exit(0)
...
for fn in sys.argv[3:]:
  ...
  for l in fi:
    ...
    r = re.match(".+> DCL/LCL/DC/LC/SC/PD:\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)", l)
    if r:
      x_dv += [float(r.group(3))]
      x_lv += [float(r.group(4))]
      x_sv += [float(r.group(5))]
      x_pd += [float(r.group(6))]
      continue
  ...
  cyc = []
  cerr = []
  for i in range(1, len(x_lv)-1):
    if (x_lv[i] < x_lv[i-1]) and (x_lv[i] < x_lv[i+1]):
      cyc += [i]
      cerr += [abs((x_lv[i-1] - x_lv[i]) - (x_lv[i+1] - x_lv[i]))]
  # determine minimum value
  lv_min = min([x_lv[i] for i in cyc])
  print(lv_min)
  # remove all non-minimal (fake) cycle points
  for i in range(len(cyc)-1,-1,-1):
    if (x_lv[cyc[i]] > 0.9*lv_min):
      print(cyc)
      cyc = cyc[0:i] + cyc[i+1:]
  ...
  data += [[name,x_t[ib:ie],x_dv[ib:ie],x_lv[ib:ie],x_sv[ib:ie],x_pd[ib:ie],
            x_h0[ib:ie],x_h1[ib:ie],x_vo[ib:ie],x_di[ib:ie]]]

# generate plot files
genplot_full("%s_drag_v_full.py" % prefix, data, 2, "Drag (Volume)")
#genplot_zoom("%s_drag_v_zoom.py" % prefix, data, 2, "Drag (Volume)", 0.13, 0.24, 3.17, 3.24)
genplot_full("%s_lift_v_full.py" % prefix, data, 3, "Lift (Volume)")
#genplot_zoom("%s_lift_v_zoom.py" % prefix, data, 3, "Lift (Volume)", 0.14, 0.19, 0.91, 1.0)
genplot_full("%s_side_v_full.py" % prefix, data, 4, "Side (Volume)")
genplot_full("%s_pdiff_full.py"  % prefix, data, 5, "Pressure Difference")
#genplot_zoom("%s_pdiff_zoom.py"  % prefix, data, 5, "Pressure Difference", 0.15, 0.24, 2.45, 2.52)
genplot_full("%s_h0_norm_u.py"   % prefix, data, 6, "H0-Norm of U")
genplot_full("%s_h1_norm_u.py"   % prefix, data, 7, "H1-Norm of U")
genplot_full("%s_vorticity.py"   % prefix, data, 8, "Vorticity")

# analyse benchmark values
print("Writing '%s_analysis.txt'..." % prefix)
fo = open("%s_analysis.txt" % prefix, "wt")
write_table2(fo, data, 2, "Drag (Volume)")
write_table1(fo, data, 3, "Lift (Volume)")
write_table1(fo, data, 4, "Side (Volume)")
write_table2(fo, data, 5, "Pressure Difference")
write_table1(fo, data, 6, "H0-Norm of U")
write_table1(fo, data, 7, "H1-Norm of U")
write_table1(fo, data, 8, "Vorticity")
write_table1(fo, data, 9, "Divergence")
fo.close()
