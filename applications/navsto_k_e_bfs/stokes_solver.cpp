...
namespace Turb
{
  ...
  bool StokesSolver::parse_args(SimpleArgParser& args)
  {
    ...
    nonlinear_system = (args.check("no-navier") < 0);
...
#if defined(FEAT_HAVE_UMFPACK) || defined(FEAT_HAVE_SUPERLU_DIST)
  ...
  void StokesSolver::create_multigrid_solver()
  {
    ...
    for(std::size_t i(0); i < stokes_levels.size(); ++i)
    {
      ...
      if((i+1) < domain.size_virtual())
      {
        ...
        //auto smoother = Solver::new_gmres(lvl.matrix_sys, lvl.filter_sys, 8, 1.0, schwarz);
      ...
      {
        auto cgsolver = Solver::new_direct_sparse_solver(lvl.matrix_sys, lvl.filter_sys);
      ...
      else
      {
        ...
        auto cgsolver = Solver::new_gmres(lvl.matrix_sys, lvl.filter_sys, 32, 0.95, schwarz);
        cgsolver->set_max_iter(500);
        cgsolver->set_tol_rel(1e-3);
        //cgsolver->set_plot_mode(Solver::PlotMode::iter);
    ...
    stokes_solver = Solver::new_gmres(stokes_levels.front()->matrix_sys,
      stokes_levels.front()->filter_sys, 8, 1.0, multigrid_precond);
  ...
  bool StokesSolver::solve_nonlinear(GlobalStokesVector& vec_sol, const GlobalStokesVector& vec_rhs)
  {
    ...
    // set plot mode to enable output
    stokes_solver->set_plot_mode(plot_nonlinear ? Solver::PlotMode::iter : Solver::PlotMode::none);
  ...
  bool StokesSolver::solve_nonlinear_turbulent(GlobalStokesVector& vec_sol, const GlobalStokesVector& vec_rhs,
    const LocalScalarVectorType& vec_k_q1, const LocalScalarVectorType& vec_e_q1)
  {
    watch_nonlinear_solve.start();
    ...
    // interpolate k and e to Q2
    LocalScalarVectorType vec_k(vec_cor.local().first().size()), vec_e(vec_cor.local().first().size());
    Assembly::FEInterpolator<SpaceTypeQ2, SpaceTypeQ1>::interpolate(vec_k, vec_k_q1, domain.front()->space_velo, domain.front()->space_turb);
    Assembly::FEInterpolator<SpaceTypeQ2, SpaceTypeQ1>::interpolate(vec_e, vec_e_q1, domain.front()->space_velo, domain.front()->space_turb);
    ...
    {
      // set up Burgers assembly job for our defect vector
      assemble_nonlinear_defect_turbulent(vec_def, vec_sol, vec_rhs, vec_k, vec_e);
      ...
      // reassemble matrix
      assemble_burgers_matrices_turbulent(vec_sol, vec_k, vec_e);
  ...
  void StokesSolver::assemble_nonlinear_defect_turbulent(GlobalStokesVector& vec_def, const GlobalStokesVector& vec_sol, const GlobalStokesVector& vec_rhs,
    const LocalScalarVectorType& vec_k, const LocalScalarVectorType& vec_e, bool filter_def)
  {
    watch_asm_vector.start();
    ...
    // set up Burgers assembly job for our defect vector
    TurbBurgersAssemblyJob<LocalMatrixBlockA, LocalVeloVector, LocalScalarVectorType, SpaceVeloType> burgers_def_job;

    // set the parameters for the Burgers assembly
    burgers_def_job.space_velo = &the_domain_level.space_velo;
    burgers_def_job.deformation = deform_tensor;
    burgers_def_job.enable_k_e = enable_turbulence;
    burgers_def_job.nu = nu;
    burgers_def_job.C_mu = C_mu;
    burgers_def_job.beta = DataType(nonlinear_system ? 1 : 0);
    burgers_def_job.theta = theta;
    burgers_def_job.vec_def_v = &vec_def.local().template at<0>();
    burgers_def_job.vec_sol_v = &vec_sol.local().template at<0>();
    burgers_def_job.vec_sol_k = &vec_k;
    burgers_def_job.vec_sol_e = &vec_e;
  ...
  void StokesSolver::assemble_burgers_matrices_turbulent(const GlobalStokesVector& vec_sol,
    const LocalScalarVectorType& vec_k, const LocalScalarVectorType& vec_e)
  {
    watch_asm_matrix.start();
    ...
    // this one will be truncated as we go down the level hierarchy
    GlobalVeloVector vec_conv(&the_stokes_level.gate_velo, vec_sol.local().template at<0>().clone());
    GlobalScalarVectorType vec_k_1(&the_stokes_level.gate_scalar_velo, vec_k.clone());
    GlobalScalarVectorType vec_e_1(&the_stokes_level.gate_scalar_velo, vec_e.clone());
    ...
    for(std::size_t i(0); i < stokes_levels.size(); ++i)
    {
      ...
      // set up Burgers assembly job for our matrix
      TurbBurgersAssemblyJob<LocalMatrixBlockA, LocalVeloVector, LocalScalarVectorType, SpaceVeloType> burgers_mat_job;

      // set the parameters for the Burgers assembly
      burgers_mat_job.space_velo = &domain.at(i)->space_velo;
      burgers_mat_job.deformation = deform_tensor;
      burgers_mat_job.enable_k_e = enable_turbulence;
      burgers_mat_job.nu = nu;
      burgers_mat_job.C_mu = C_mu;
      burgers_mat_job.beta = DataType(nonlinear_system ? 1 : 0);
      burgers_mat_job.beta_frechet = DataType(nonlinear_system && newton_solver ? 1 : 0);
      burgers_mat_job.theta = theta;
      burgers_mat_job.matrix_a = &loc_mat_a;
      burgers_mat_job.vec_sol_v = &vec_conv.local();
      burgers_mat_job.vec_sol_k = &vec_k_1.local();
      burgers_mat_job.vec_sol_e = &vec_e_1.local();
      ...
      {
        // create a coarse mesh velocity vector
        auto vec_crs_v = stokes_levels.at(i+1)->matrix_a.create_vector_l();
        //auto vec_crs_k = stokes_levels.at(i+1)->matrix_a.create_vector_l();
        //auto vec_crs_e = stokes_levels.at(i+1)->matrix_a.create_vector_l();
        GlobalScalarVectorType vec_crs_k(&stokes_levels.at(i+1)->gate_scalar_velo, LocalScalarVectorType(vec_crs_v.local().size()));
        GlobalScalarVectorType vec_crs_e(&stokes_levels.at(i+1)->gate_scalar_velo, LocalScalarVectorType(vec_crs_v.local().size()));

        // truncate fine mesh velocity vector
        stokes_levels.at(i)->transfer_velo.trunc(vec_conv, vec_crs_v);
        stokes_levels.at(i)->transfer_scalar_velo.trunc(vec_k_1, vec_crs_k);
        stokes_levels.at(i)->transfer_scalar_velo.trunc(vec_e_1, vec_crs_e);

        // the coarse vector is our next convection vector
        vec_conv = std::move(vec_crs_v);
        vec_k_1 = std::move(vec_crs_k);
        vec_e_1 = std::move(vec_crs_e);
      }
      else
      {
        // this process is a child, so send truncation to parent
        stokes_levels.at(i)->transfer_velo.trunc_send(vec_conv);
        stokes_levels.at(i)->transfer_scalar_velo.trunc_send(vec_k_1);
        stokes_levels.at(i)->transfer_scalar_velo.trunc_send(vec_e_1);
      }
    }
    watch_asm_matrix.stop();
  }
