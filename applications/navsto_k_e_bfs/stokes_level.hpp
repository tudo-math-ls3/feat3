...
namespace Turb
{
  ...
  public:
    ...
    /**
     * \brief Extrapolates solution vectors of a previous time-step to obtain an initial solution
     *
     * \param[inout] vec_sol
     * The vector that should receive the extrapolated initial guess
     *
     * \param[in] expolc
     * The extrapolation factors from the time stepping scheme.
     *
     * \param[in] vec_sol_1, vec_sol_2, vec_sol_3
     * The solution vectors of the last three time steps.
     */
    template<int n_>
    void extrapolate_sol(GlobalStokesVector& vec_sol, const Tiny::Vector<DataType, n_>& expolc,
      const GlobalStokesVector& vec_sol_1, const GlobalStokesVector& vec_sol_2, const GlobalStokesVector& vec_sol_3)
    {
      static_assert(n_ >= 4, "invalid expolc size");
     ...
     */
    template<int n_>
    void update_unsteady_rhs(GlobalStokesVector& vec_rhs, const Tiny::Vector<DataType, n_>& theta,
      const GlobalStokesVector& vec_sol_1, const GlobalStokesVector& vec_sol_2)
    {
      static_assert(n_ >= 3, "invalid theta size");

      LocalVeloVector vec_tmp(vec_rhs.local().first().size());
      vec_tmp.scale(vec_sol_1.local().first(), theta[1]);
      if(Math::abs(theta[2]) > 0.0)
        vec_tmp.axpy(vec_sol_2.local().first(), theta[2]);
      local_velo_mass_matrix.apply(vec_rhs.local().first(), vec_tmp, vec_rhs.local().first());
    }
  }; // class StokesLevel
