...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
// This is the base header for all applications in this directory. This header defines the most basic classes used
// throughout the entire code base, such as mesh types, FE spaces and linear algebra containers, as well as some
// auxiliary helper functions. This header also defines whether the application is compiled as a 2D or a 3D application
// by checking for the definition of the FEAT_CCND_SIMPLE_3D preprocessor macro. This header is included by all other
// header and source files in this directory, either directly or indirectly.
//
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
namespace Turb
{
  ...
  /// the turbulence space type
  typedef Space::Lagrange1::Element<TrafoType> SpaceTurbType;
  ...
  typedef Global::Vector<LocalScalarVectorType, MirrorType> GlobalScalarVectorType;


  //inline String stringify_percent(double part, double total)
  //{
    //return " [" + stringify_fp_fix(100.0*part/total, 2, 6)
  //}
   ...
   */
  inline void print_pad(const Dist::Comm& comm, const String& key, const String& value, int pad_len = 42, char pad_char = '.')
  {
   ...
   * The character to pad with.
   */
  inline void print_time(const Dist::Comm& comm, const String& key, double time, double total_runtime, int pad_len = 42, char pad_char = '.')
  {
    //print_pad(comm, key, stringify_fp_fix(time, 3, 10) + " [" + stringify_fp_fix(100.0*time/total_runtime, 2, 6) + "%]", pad_len, pad_char);
   ...
   * The communicator
   */
  inline void print_memory_usage(const Dist::Comm& comm)
  {
    MemoryUsage mi;
    ...
    print_pad(comm, "Peak Physical Memory Usage per Process", stringify_bytes(mem_max[0], 3, 10)
      + " { " + stringify_fp_fix(mem_min[0] > 0u ? double(mem_min[0])/double(mem_max[0]) : 0.0, 3, 5) + " }");
    print_pad(comm, "Peak Virtual Memory Usage per Process", stringify_bytes(mem_max[1], 3, 10)
      + " { " + stringify_fp_fix(mem_min[1] > 0u ? double(mem_min[1])/double(mem_max[1]) : 0.0, 3, 5) + " }");
    print_pad(comm, "Total Peak Physical Memory Usage", stringify_bytes(mem_sum[0], 3, 10));
    print_pad(comm, "Total Peak Virtual Memory Usage",  stringify_bytes(mem_sum[1], 3, 10));
  }
} // namespace Turb
