...
namespace Turb
{
  ...
  bool VtkWriter::register_scalar_vector(const String& name, GlobalScalarVectorType& vector)
  {
    return scalar_vectors.emplace(name, &vector).second;
  }
  ...
  void VtkWriter::add_lagrange1_vector(const LocalScalarVectorType& vector, const String& name)
  {
    if(refined_mesh)
    {
      ...
      Assembly::FEInterpolator<SpaceVeloType, SpaceTypeQ1>::interpolate(
          vector2, vector, domain.front()->space_velo, domain.front()->space_turb);
  ...
  bool VtkWriter::write_registered(const TimeStepping& time_stepping)
  {
    ...
    if(prepare_write(time_stepping.time_step))
    {
      ...
      for(const auto& x : scalar_vectors)
        add_lagrange1_vector(x.second->local(), x.first);
