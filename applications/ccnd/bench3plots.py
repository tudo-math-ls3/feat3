...
#
# This script analyzes a (non-empty) sequence of output logfiles generated by the ccnd-unsteady-dfg95-2d/3d application
...
#
# Typically, one wants to analyze the convergence behavior of several simulation runs which only differ on the mesh
...
#
# would analyze the convergence for various mesh refinement levels (3,4,5,6) for the same time step size k=1/400,
# whereas the call
#
# > bench3plots.py lvl=5_tsteps lvl=5_k=100.log lvl=5_k=200.log lvl=5_k=400.log lvl=5_k=800.log
#
# would analyze the convergence for various time step sizes (k=1/100,k=1/200,k=1/400,k=1/800) on the same mesh.
#
# \author Peter Zajac
########################################################################################################################
import os
...
for fn in sys.argv[2:]:
  ...
  for l in fi:
    ...
    if(c_t > 8.0):
      continue
    # new format
    r = re.match(".+> Line Forces.+:\s+(\S+)\s+(\S+)", l)
    if r:
      x_dl += [float(r.group(1))]
      x_ll += [float(r.group(2))]
      continue
    r = re.match(".+> Volume Forces.+:\s+(\S+)\s+(\S+)\s+(\S+)", l)
    if r:
      x_dv += [float(r.group(1))]
      x_lv += [float(r.group(2))]
      continue
    r = re.match(".+> Pressure Difference.+:\s+(\S+)", l)
    if r:
      x_pd += [float(r.group(1))]
    r = re.match(".+> Fluxes Upper/Lower/Out:\s+(\S+)\s+(\S+)\s+(\S+)", l)
    if r:
      x_fu += [float(r.group(1))]
      x_fl += [float(r.group(2))]
      x_fo += [float(r.group(3))]
      continue
    r = re.match(".+> Velocity H0/H1/Vor/Div:\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)", l)
    if r:
      x_h0 += [float(r.group(1))]
      x_h1 += [float(r.group(2))]
      x_vo += [float(r.group(3))]
      x_di += [float(r.group(4))]
      continue
    r = re.match(".+> Velocity Time-Der Norm:\s+(\S+)", l)
    if r:
      x_t  = x_t  + [c_t]
      x_vt += [float(r.group(1))]
      continue
    # old format ("dfg95_bench" apps)
    ...
    if r:
      x_fu += [2.0*float(r.group(1))]
      x_fl += [2.0*float(r.group(2))]
  ...
  if fn.endswith(".log"):
    ...
    name = fn
  data += [[name,x_t,x_dl,x_ll,x_dv,x_lv,x_pd,x_fu,x_fl,x_fo,x_h0,x_h1,x_vo,x_di,x_vt]]
...
genplot_full("%s_lift_l_full.py" % prefix, data, 3, "Lift (Line)")
genplot_zoom("%s_lift_l_zoom.py" % prefix, data, 3, "Lift (Line)", 5.65, 5.75, 0.4, 0.5)
genplot_full("%s_drag_v_full.py" % prefix, data, 4, "Drag (Volume)")
genplot_zoom("%s_drag_v_zoom.py" % prefix, data, 4, "Drag (Volume)", 3.5, 4.3, 2.85, 3.0)
genplot_zoom("%s_drag_v_zoom_2.py" % prefix, data, 4, "Drag (Volume)", 5.2, 5.6, 2, 2.5)
...
genplot_full("%s_flux_o_full.py" % prefix, data, 9, "Out Flux")
genplot_full("%s_h0_norm_u.py"   % prefix, data,10, "H0-Norm of U")
genplot_full("%s_h1_norm_u.py"   % prefix, data,11, "H1-Norm of U")
genplot_full("%s_vorticity.py"   % prefix, data,12, "Vorticity")
genplot_full("%s_velo_der.py"    % prefix, data,14, "Velocity Derivative")
...
write_table(fo, data, 3, "Lift (Line)")
write_table(fo, data, 4, "Drag (Volume)")
...
write_table(fo, data, 9, "Out Flux")
write_table(fo, data,10, "H0-Norm of U")
write_table(fo, data,11, "H1-Norm of U")
write_table(fo, data,12, "Vorticity")
write_table(fo, data,13, "Divergence")
write_table(fo, data,14, "Time Derivative")
fo.close()
