...
#define FEAT_DFG95_BENCH1_DIM SCALEXA_DIM
#define FEAT_DFG95_BENCH1_DOUBLE 1
...
#error FEAT_DFG95_BENCH1_QUAD_DOUBLE: use quadruple for system and double for solver
  ...
  struct Times
  {
    ...
    static constexpr std::size_t scalar_coarse_helper_init_symbolic = 16u;
    static constexpr std::size_t scalar_coarse_helper_init_numeric = 17u;
    static constexpr std::size_t scalar_coarse_solver_init_symbolic = 18u;
    static constexpr std::size_t scalar_coarse_solver_init_numeric = 19u;
    static constexpr std::size_t scalar_coarse_solver_apply = 20u;
    static constexpr std::size_t count = 21u;
  };
  ...
  static constexpr std::size_t padlen = std::size_t(40);
  ...
  public:
    ...
    String format() const
    {
      ...
      s += format_subtime_mm("Scalar Coarse Helper Init Symbolic", times[Times::scalar_coarse_helper_init_symbolic], times_min[Times::scalar_coarse_helper_init_symbolic], times_max[Times::scalar_coarse_helper_init_symbolic]);
      s += format_subtime_mm("Scalar Coarse Helper Init Numeric", times[Times::scalar_coarse_helper_init_numeric], times_min[Times::scalar_coarse_helper_init_numeric], times_max[Times::scalar_coarse_helper_init_numeric]);
      s += format_subtime_mm("Scalar Coarse Solver Init Symbolic", times[Times::scalar_coarse_solver_init_symbolic], times_min[Times::scalar_coarse_solver_init_symbolic], times_max[Times::scalar_coarse_solver_init_symbolic]);
      s += format_subtime_mm("Scalar Coarse Solver Init Numeric", times[Times::scalar_coarse_solver_init_numeric], times_min[Times::scalar_coarse_solver_init_numeric], times_max[Times::scalar_coarse_solver_init_numeric]);
      s += format_subtime_mm("Scalar Coarse Solver Apply", times[Times::scalar_coarse_solver_apply], times_min[Times::scalar_coarse_solver_apply], times_max[Times::scalar_coarse_solver_apply]);
...
int main(int argc, char* argv[])
{
  ...
  std::shared_ptr<Solver::Trilinos::FROSchParameterList> params = std::make_shared<Solver::Trilinos::FROSchParameterList>(comm, dim, Solver::Trilinos::FROSchParameterList::SADDLEPOINT);
  params->parse_args(args);
  params->create_core();

  Index krylov_dim = 10;
  SolverDataType tol_rel = 1E-5;
  SolverDataType tol_abs = 1E-8;
  SolverDataType inner_res_scale = 1.;
  Index krylov_maxit = 1000;
  Index krylov_minit = 1;
...

  {
    ...
    comm.print(String("Coarse Grid Solver").pad_back(pl, pc) + ": scalarized solver");
  }
  ...
  const String cubature_postproc_string("gauss-legendre:5");
  Cubature::DynamicFactory cubature_postproc(cubature_postproc_string);
  ...
  for(std::size_t i(0); i < solver_levels.size(); ++i)
  {
    ...
    else
    {
...
#ifdef FEAT_HAVE_TRILINOS
      {
        ...
        solver_iterative->set_min_iter(krylov_minit);
        solver_iterative->set_max_iter(krylov_maxit);
        if(krylov_info)
...
#if 0
      {
        auto solver_iterative = Solver::new_fgmres(scalarize_helper.scalarized_level.matrix_sys, scalarize_helper.scalarized_level.filter_sys, 8, 1.0);
        solver_iterative->set_tol_rel(1E-2);
        solver_iterative->set_tol_abs(1E+99);
        solver_iterative->set_tol_abs_low(1e-14);
        solver_iterative->set_max_iter(16);
        solver_iterative->set_plot_name("FGMRES<" + stringify(comm.rank()) + ">");
        solver_iterative->set_plot_mode(Solver::PlotMode::iter);
  ...
  if(!skip_analysis)
  {
    ...
    Assembly::TraceAssemblyStokesBodyForceAssemblyJob
      job(vec_sol.local().template at<0>(), vec_sol.local().template at<1>(), the_domain_level.space_velo, the_domain_level.space_pres,
          cubature_postproc_string);
    body_force_asm.assemble(job);
    job.sync(comm);

    summary.body_forces_raw_surf[0] = job.drag(nu);
    summary.body_forces_raw_surf[1] = job.lift(nu);
    summary.body_forces_raw_surf[2] = job.side(nu);
    summary.drag_coeff_surf_cylinder = summary.body_forces_raw_surf[0] * dpf2_cylinder;
    summary.lift_coeff_surf_cylinder = summary.body_forces_raw_surf[1] * dpf2_cylinder;
    summary.drag_coeff_surf_sphere = summary.body_forces_raw_surf[0] * dpf2_sphere;
    summary.lift_coeff_surf_sphere = summary.body_forces_raw_surf[1] * dpf2_sphere;
  }
  ...
  if(!skip_analysis)
  {
    ...
    auto fx_in = Assembly::integrate_discrete_function<0>(flux_asm_in, vec_sol.local().template at<1>(), the_domain_level.space_pres, cubature_postproc_string);
    auto fx_out = Assembly::integrate_discrete_function<0>(flux_asm_out, vec_sol.local().template at<1>(), the_domain_level.space_pres, cubature_postproc_string);

    fx_in.synchronize(comm);
    fx_out.synchronize(comm);


    // compute pressure drop average over inflow area
    summary.pres_drop_raw = fx_in.value - fx_out.value;
    summary.pres_drop_pipe = (summary.pres_drop_raw) / (dim == 2 ? SystemDataType(0.41) : SystemDataType(0.205*0.205)*Math::pi<SystemDataType>());
    summary.pres_drop_cuboid = (summary.pres_drop_raw) / (dim == 2 ? SystemDataType(0.41) : SystemDataType(0.41*0.41));
  }

  // compute flux through region above cylinder
  if(!skip_analysis)
  {
    auto fx_u = Assembly::integrate_discrete_function<0>(flux_asm_u, vec_sol.local().template at<0>(), the_domain_level.space_velo, cubature_postproc_string);
    fx_u.synchronize(comm);

    summary.flux_upper = fx_u.value[0] / SystemDataType(2);
  }

  // compute flux through region below cylinder
  if(!skip_analysis)
  {
    auto fx_l = Assembly::integrate_discrete_function<0>(flux_asm_l, vec_sol.local().template at<0>(), the_domain_level.space_velo, cubature_postproc_string);
    fx_l.synchronize(comm);

    summary.flux_lower = fx_l.value[0] / SystemDataType(2);
  }
