...
struct MeshoptScrewsApp
{
  ...
  const bool write_vtk, const Index vtk_freq, const bool test_mode)
  {
    ...
    if(test_mode)
    {
      if( Math::abs(worst_angle - DT_(7.618381)) > Math::pow(Math::eps<DT_>(), DT_(0.25)))
      {
        Util::mpi_cout("FAILED:");
        throw InternalError(__func__,__FILE__,__LINE__,
        "Initial worst angle should be >= "+stringify_fp_fix(7.618381)+ " but is "+stringify_fp_fix(worst_angle));
      }
    }
    ...
    if(test_mode)
    {
      if( Math::abs(worst_angle - DT_(7.618381)) > Math::pow(Math::eps<DT_>(), DT_(0.25)))
      {
        Util::mpi_cout("FAILED:");
        throw InternalError(__func__,__FILE__,__LINE__,
        "Post initial worst angle should be >= "+stringify_fp_fix(7.618381)+ " but is "+stringify_fp_fix(worst_angle));
      }
    }
    ...
    if(test_mode)
    {
      if( Math::abs(worst_angle - DT_(7.592201)) > Math::pow(Math::eps<DT_>(), DT_(0.25)))
      {
        Util::mpi_cout("FAILED:");
        throw InternalError(__func__,__FILE__,__LINE__,
        "Final worst angle should be >= "+stringify_fp_fix(7.605283)+ " but is "+stringify_fp_fix(worst_angle));
      }
    }
