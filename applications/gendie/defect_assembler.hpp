...
namespace Gendie
{
  namespace Intern
  {
    ...
    struct DefectSaddlePointHelper
    {
      typedef SystemLevel_ LevelType;
      static constexpr bool fbm_support = Intern::supports_fbm<SystemLevel_>;

      template<typename DefectVector_, typename PrimalVector_>
      static void assemble_defect(
        DefectVector_& vec_def,
        const PrimalVector_& vec_primal,
        const SystemLevel_& cur_sys
      )
      {
        typedef typename DefectVector_::DataType DataType;
        if constexpr(fbm_support)
      ...
      static void assemble_defect(
        ...
        const PrimalVector_& vec_primal,
        const SystemLevel_& cur_sys,
        StopWatch& watch_fbm_filter,
        StopWatch& watch_grad_apply
      )
      {
        typedef typename DefectVector_::DataType DataType;
   ...
   *
   */
  template<typename Domain_, typename SystemLevel_>
  ...
  public:
    ...
    static constexpr bool fbm_support = BaseClass::fbm_support;
    friend BaseClass;
  ...
  protected:
    ...
    void _assemble_defect(VectorType& vec_def, const VectorType& vec_primal, const VectorType& vec_conv, const VectorType& vec_rhs) const
    {
      ...
      FEAT::Assembly::BurgersVeloMaterialBlockedVectorAssemblyJob burgers_vec_job(
        ...
        this->_cubature, this->_material->get_visc_func(_mesh_scale), this->_material->get_visc_der_func(_mesh_scale)
      );
      {
      ...
      watch_assembly.stop();
      Intern::DefectSaddlePointHelper<LevelType>::assemble_defect(vec_def, vec_primal, cur_sys, watch_fbm_filter, watch_grad_apply);
   ...
   */
  template<typename Domain_, typename SystemLevel_>
  class BurgersDefectFlowAssembler : public DefectFlowAssemblerBaseCRTP<BurgersDefectFlowAssembler<Domain_, SystemLevel_>, Domain_, SystemLevel_>
  {
  public:
    typedef DefectFlowAssemblerBaseCRTP<BurgersDefectFlowAssembler, Domain_, SystemLevel_> BaseClass;
    typedef typename BaseClass::DomainType DomainType;
    typedef typename BaseClass::LevelType LevelType;
    typedef typename BaseClass::VectorType VectorType;
    typedef typename BaseClass::DataType DataType;
  ...
  protected:
    ...
    void _assemble_defect(VectorType& vec_def, const VectorType& vec_primal, const VectorType& vec_conv, const VectorType& vec_rhs) const
    {
      ...
      const auto& cur_sys = *this->system_level;
      auto& cur_dom = *this->domain->front();
      FEAT::Assembly::BurgersBlockedVectorAssemblyJob burgers_vec_job(
        vec_def.local().template at<0>(), vec_primal.local().template at<0>(), vec_conv.local().template at<0>(), cur_dom.space_velo, this->_cubature
      );
      {
        burgers_vec_job.deformation = deformation;
        burgers_vec_job.nu = nu;
        burgers_vec_job.beta = beta;
        burgers_vec_job.theta = theta;
        burgers_vec_job.alpha = DataType(-1) / this->stiffness_factor;
      }
      vec_def.format();
      // actual assembly
  ...
  public:
    DataType nu, beta, theta;
    bool deformation;


  public:
    explicit BurgersDefectFlowAssembler()
      : BaseClass(),
        nu(DataType(1)),
        beta(DataType(0)),
        theta(DataType(0)),
    ...
    explicit BurgersDefectFlowAssembler(DomainType& domain_, const std::shared_ptr<SystemLevel_>& system_level_, const FEAT::PropertyMap* prop)
      : BaseClass(domain_, system_level_, prop),
        nu(DataType(1)),
        beta(DataType(0)),
        theta(DataType(0)),
    ...
    virtual ~BurgersDefectFlowAssembler() = default;

    BurgersDefectFlowAssembler(const BurgersDefectFlowAssembler&) = default;
    BurgersDefectFlowAssembler(BurgersDefectFlowAssembler&&) noexcept = default;
    BurgersDefectFlowAssembler& operator=(const BurgersDefectFlowAssembler&) = default;
    BurgersDefectFlowAssembler& operator=(BurgersDefectFlowAssembler&&) noexcept = default;

  }; //class BurgersDefectFlowAssembler<...>
}
